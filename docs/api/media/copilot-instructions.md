...
Ollama and Cline alignment truth: keep the governed local stack aligned on `http://127.0.0.1:11434`, `gemma2:2b`, canonical IPC, and no token gate. Cline safeguards may require explicit user request for critical builds/deploys but must never contradict Rule 11. Any Ollama/Cline config change must rerun the dedicated alignment validator and update `OLLAMA_RUNTIME_MAP.md` plus the relevant instructions/docs.
