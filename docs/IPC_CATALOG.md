...
> 2026-05-27 — Remote Gateway `conversation_generate` contract correction: la voie HTTP `POST /api/invoke` vers `conversation_generate` attend le payload camelCase `{ message, conversationId, contextEnvelope?, mode?, provider?, systemPrompt? }`, aligne sur `ConversationGenerateArgs` (`serde(rename_all = "camelCase")`). Les anciens exemples `conversation_id` sont invalides et produisent `invalid args: missing field 'conversationId'`. La SPA remote `src/remote/useRemoteChat.ts` et `tests/contract/remote-chat-contract.test.ts` ont ete realignes.
...
> 2025-07-15 — Remote Gateway Phase 2 (v31.2.3): `POST /api/invoke → conversation_generate` maintenant opérationnel via `conversation_generate_inner()` (src-tauri/src/conversation_engine/commands.rs). `GatewayState` contient `engine: Arc<ConversationEngineState>` + `orchestrator: ChatOrchestratorState`. Payload canonique: `ConversationGenerateArgs { message, conversationId, mode, provider, systemPrompt, contextEnvelope, ... }` (serde camelCase). Retourne `{ ok, content: { content|response, conversationId, meta: { provider_used, latency_ms_total, ... } } }`. Browser UI: `RemoteLoginPage` (data-testid=remote-login-page) + `RemoteGatewayLayout` (data-testid=remote-gateway-layout). Tests: `tests/contract/remote-chat-contract.test.ts` (40 Vitest) + `e2e/remote-chat-api.spec.ts` + `e2e/remote-chat-browser.spec.ts` (Playwright). `invoke()` retourne `content` directement et throw sur `ok:false`.
