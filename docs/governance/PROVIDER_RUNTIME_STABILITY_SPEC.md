...
## 7. Confirmed breakpoint + fix candidate (2026-03-28)
- Confirmed breakpoint: `BREAK_AT_PROVIDER_EXECUTION` with `PROVIDER_UNAVAILABLE` on embedded runtime when assets were stale.
- Fix candidate class: `HARNESS_RUNTIME_SYNC_GLITCH` (embedded assets out of sync with source).
- Applied bounded fix: rebuild Tauri e2e bundle (no code change).
- Result: provider execution OK in at least one run; x3 stability blocked by WRY session invalid id.

## 8. Minimal fix rules
Only small, bounded fixes are allowed here (no provider fabric/control-plane redesign).

## 9. Reopen / triage rule
