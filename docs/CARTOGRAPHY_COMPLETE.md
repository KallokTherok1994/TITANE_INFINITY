...
> 2026-04-17 — Security audit desktop runtime proof truth: les commandes kernel `security_audit_sync_journal` et `security_audit_publish_signed_export` sont maintenant réellement disponibles dans le binaire debug desktop courant du workspace, ce qui permet à la preuve WDIO native d écrire `federated_audit_journal.json`, `governed_export_signing_key.json` et un export signé sous `~/.local/share/com.titane.infinity/security_active/`. La surface installée `/usr/bin/titane-infinity` reste non alignée tant qu un build/distribuable gouverné n a pas été relancé.
