...
> 2026-04-30 — AI intelligence unification v32 truth: le noyau identitaire `buildTitaneIdentityPromptBlock()` est désormais propagé dans les 20 modes actifs de `src/services/ai/chatModes.config.ts` (seuls `htf_soumission` et `kalloks_arts` sont protégés), dans les 5 modes legacy de `src/services/ai/chatModes.ts`, et comme bloc de fondation dans `src/core/prompts/profiles.ts` (`CORE_SYSTEM_PROMPT`). Le protocole Chain-of-Thought analytique est injecté dans 6 modes (`reflection`, `debug_cognitive`, `strategy`, `audit`, `dev`, `omega`). `BackendConversationMode` est étendu de 6 à 13 valeurs (ajout `strategy`, `dev`, `omega`, `audit`, `reflection`, `decision`, `veille_recherche`) dans `src/services/ai/omegaModeClassifier.ts`, `src/services/conversationEngine.ts`, et côté Rust dans `src-tauri/src/conversation_engine/types.rs` + `commands.rs` + `pipeline.rs`. `ChatMode` dans `src/services/ai/chatTypes.ts` et `CommunicationTone` dans `src/types/chatModes.ts` sont alignés. Le mapping `CANONICAL_MODE_SPECS` dans `omegaModeClassifier.ts` est mis à jour (`DEEP_REASONING` → `reflection`, `ARCHITECT` → `strategy`, `CERTIFY` → `audit`). `pnpm run check` et `cargo check` : PASS.
