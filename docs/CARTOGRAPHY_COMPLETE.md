...
- **Système / Update prompt** : nouveau composant [src/components/system/UpdateAvailableToast.tsx](src/components/system/UpdateAvailableToast.tsx) — listener `window.addEventListener('sw-update-available', …)` émis par `ServiceWorkerManager.notifyUpdateAvailable()`, déclenche `sonner.toast.info` avec action `Recharger` qui post `SKIP_WAITING` au waiting worker (le `controllerchange` listener existant gère le reload). Wiré dans `src/App.tsx` à côté de `GlobalRuntimePulse` / `BackendDownIndicator`.
- **Surface Truth canonique (overlay diagnostic)** : nouveau couple [src/components/dev/SurfaceTruthBadge.tsx](src/components/dev/SurfaceTruthBadge.tsx) + [src/hooks/useSurfaceTruth.ts](src/hooks/useSurfaceTruth.ts) — toggle Ctrl+Alt+T (auto-activé en DEV ou via `localStorage.titane_surface_truth='1'`) exposant : `appVersion` (`__APP_VERSION__`), `buildTimestamp` (`__BUILD_TIMESTAMP__`), `storeVersion` Zustand, `swScope`, `swController`, `transport` (tauri/web), `chunkHash` (issu de `import.meta.url`), `data-surface-truth` détecté dans le DOM. Wiré dans `src/App.tsx`.
- **Build truth** : [vite.config.ts](vite.config.ts) `define` étendu avec `__BUILD_TIMESTAMP__: JSON.stringify(new Date().toISOString())`; typage dans [src/vite-env.d.ts](src/vite-env.d.ts).
- **Zustand persist** : [src/core/state/SingularityState.ts](src/core/state/SingularityState.ts) — bump `name: 'titane-singularity-state-v19'` → `'titane-singularity-state-v34'`, ajout `version: 34` + `migrate(persistedState, fromVersion)` défensif qui, pour `fromVersion < 34`, ne conserve que `ui` / `context` (avec `page: 'dashboard'` reset) / `metaMode` et drop `ai / engines / enginesData / globalHealth` (futur scope TanStack Query, Phase 6 du plan v35).
- **Scripts d'audit déterministes (Phase 5.6 + 5.7 du plan v35)** :
  - [scripts/audit/orphan-pages.mjs](scripts/audit/orphan-pages.mjs) — parse `<Route path=…>` et `<Navigate to=…>` dans `src/App.tsx` + walk `src/pages/**`, écrit `reports/ui-orphan-pages.{json,md}`. Premier run : **96 routes** (29 element, 67 redirect = **69.8 %** legacy), **72** fichiers pages, breakdown `MOUNTED_VISIBLE=10 / MOUNTED_HIDDEN=13 / ORPHAN_DEAD=49`. Mode `--strict` exit 1 si ORPHAN_DEAD > 0.
  - [scripts/audit/sync-allowed-commands.mjs](scripts/audit/sync-allowed-commands.mjs) — diff `tauri::generate_handler![…]` (`src-tauri/src/main.rs`) ↔ Remote Gateway match arms (`src-tauri/src/remote_gateway/handlers.rs`) ↔ `ALLOWED_COMMANDS` (`src/lib/security.ts`), écrit `reports/ipc-allowed-commands.{json,md}`. Premier run : **drift confirmé** (frontend ALLOWED_COMMANDS = 861, à rapprocher des handlers Rust effectifs).
  - npm scripts ajoutés : `audit:orphan-pages`, `audit:orphan-pages:strict`, `audit:allowed-commands`, `audit:allowed-commands:strict`.
- **Tests Vitest PASS (9/9)** :
  - [src/__tests__/components/system/UpdateAvailableToast.test.tsx](src/__tests__/components/system/UpdateAvailableToast.test.tsx) — 4 tests (mount silencieux, render après event, action `onReload`, déduplication multi-events).
  - [src/__tests__/components/dev/SurfaceTruthBadge.test.tsx](src/__tests__/components/dev/SurfaceTruthBadge.test.tsx) — 3 tests (disabled, toggle open, defaultOpen).
  - [src/__tests__/core/state/SingularityState.migrate.test.ts](src/__tests__/core/state/SingularityState.migrate.test.ts) — 2 tests (clé v34, drop pré-v34).
- **AutoHeal** : entrée `UI-STALE-PAGES-NETWORKFIRST-v34_0_13-2026-04-16` (schéma complet, `prevention_test` inclut `detect_recurrence`). Gates `bash scripts/autoheal/detect_recurrence.sh` et `bash scripts/verify/verify-copilot-instructions.sh` → **PASS**.
- **Scope hors v34.0.13** (déféré aux phases suivantes du plan v35) : RemoteGateway extension `web_search` / `memory_*` (Phase 4), TanStack Query migration des dynamic fields (Phase 6), audit fixers `--fix` (Phase 7), Tauri 2 webview `data-cache: NoCache` (Phase 5.4).
...
- `src/hooks/useTitaneCore.ts` now uses the allowlisted `get_helios_metrics` command and accepts both object and JSON string payloads for Helios/Nexus/Harmonia fetches; `src/test/useTitaneCore.test.ts` locks the command name and object-payload path.
...
> 2026-05-27 — Remote Gateway browser LAN login truth: la SPA remote servie sur `http://127.0.0.1:7420` peut maintenant authentifier une gateway saisie en LAN (`http://192.168.2.16:7420`) sans blocage CSP. Le header CSP Axum remote ouvre `connect-src` aux schemes HTTP/HTTPS/WS/WSS pour cette surface de gateway/tunnel, tandis que la CSP frontend generale reste `'self'` pour conserver la voie One Door vers Ollama. `useRemoteChat` envoie le payload canonique `conversationId`/`contextEnvelope` vers `conversation_generate`, lit aussi `content` dans la reponse, et `/favicon.ico` repond `204`. Preuves: `pnpm run check`, Vitest remote+CSP 19 PASS, `pnpm run build:remote`, Playwright LAN manual sans console error, `e2e/remote-chat-browser.spec.ts` 8 PASS, `e2e/remote-gateway.spec.ts` 16 PASS. Rollback: restaurer les fichiers remote/gateway/CSP/tests et retirer cette entree cartographie.
...
> La voie streaming Ollama du runtime `src-tauri/src/overdrive/chat_orchestrator.rs` aligne aussi la vérité `requested -> used -> shown`: le modèle final publié aux événements Tauri et au message assistant ne reste plus figé sur la requête initiale si le chunk terminal Ollama annonce une autre valeur runtime.
...
> 2026-05-02 — Chat modes profile coverage truth: `src/services/ai/responsePolicy.ts::MODE_PROFILE_MAP` couvre maintenant explicitement tous les modes actifs. Avant correction, `htf_soumission`, `psychologie_profils`, `humain_total` et `kalloks_arts` dérivaient vers le fallback implicite `BALANCED`, malgré des prompts et usages spécialisés.
> 2026-05-02 — Chat non-stream mode-cap truth: la voie `chatEngine.generate() -> aiOrchestrator.generate()` n utilise plus le plafond brut du profil kernel quand le mode actif a déjà imposé un cap explicite. `src/services/ai/chatEngine.ts` forwarde maintenant `effectiveResponseProfile.maxTokens` et `effectiveResponseProfile.temperature`, ce qui ferme la dernière fuite runtime entre configuration de mode et génération effective.

> 2026-05-02 — Chat context budget truth: la surface active `src/components/chat/ContextUsage.tsx` et le backend de calcul `src/services/ai/contextManager.ts` convergent maintenant sur la même fenêtre locale gouvernée pour `gemma2:2b` (`8192` tokens). Cette correction supprime un drift où la jauge de contexte pouvait retomber sur un plafond cloud `gpt-4-turbo` (`128000`) pour le champion Ollama local, ce qui faussait la lecture utilisateur du budget de conversation.
...
> 2026-04-30 — AI intelligence unification v32 truth: le noyau identitaire `buildTitaneIdentityPromptBlock()` est désormais propagé dans les 20 modes actifs de `src/services/ai/chatModes.config.ts` (seuls `htf_soumission` et `kalloks_arts` sont protégés), dans les 5 modes legacy de `src/services/ai/chatModes.ts`, et comme bloc de fondation dans `src/core/prompts/profiles.ts` (`CORE_SYSTEM_PROMPT`). Les couches analytiques conservent leur discipline cognitive élevée, mais la narration explicite du Chain-of-Thought est désormais contenue: `src/services/ai/chatEngine.ts`, `src/services/userPreferencesEngine.ts`, `src/services/ai/chatModes.ts`, `src/services/ai/chatModes.config.ts`, `src/config/chatModes.config.ts` et `src/ui/pages/ChatIA/InstructionModeManager.ts` orientent maintenant la restitution vers un langage plus humain, vivant, direct et crédible, en gardant les phases et protocoles en interne sauf demande explicite. `BackendConversationMode` est étendu de 6 à 13 valeurs (ajout `strategy`, `dev`, `omega`, `audit`, `reflection`, `decision`, `veille_recherche`) dans `src/services/ai/omegaModeClassifier.ts`, `src/services/conversationEngine.ts`, et côté Rust dans `src-tauri/src/conversation_engine/types.rs` + `commands.rs` + `pipeline.rs`. `ChatMode` dans `src/services/ai/chatTypes.ts` et `CommunicationTone` dans `src/types/chatModes.ts` sont alignés. Le mapping `CANONICAL_MODE_SPECS` dans `omegaModeClassifier.ts` est mis à jour (`DEEP_REASONING` → `reflection`, `ARCHITECT` → `strategy`, `CERTIFY` → `audit`). `pnpm run check` et `cargo check` : PASS.
...
> 2026-05-18 — Progression/XP canonical fusion truth: `/progression` pointe maintenant vers `/titane?tab=progression`, et toutes les surfaces Progression/XP actives derivent leurs niveaux, totaux, compteurs chat et milestones de `ExperienceState` via `experienceService` + `xpCanonical`. `src/cognitive/progression/xpEngine.ts` reste un adaptateur compatible, mais ne maintient plus `titane_progression_state`; `src/core/experience/XP_ENGINE.ts` reste un miroir legacy sans persistance `xp_state`. La preuve E2E ciblee `e2e/features/progression-xp-fusion.spec.ts` verifie la route alias, `/experience` et le compteur chat partage.
...
> 2026-04-30 — Chat natural-expression depth truth: les surfaces prompt actives `src/services/ai/chatEngine.ts`, `src/services/ai/chatModes.ts`, `src/services/userPreferencesEngine.ts`, `src/config/chatModes.config.ts` et le fallback UI `src/ui/pages/ChatIA/InstructionModeManager.ts` gardent un niveau d exigence élevé sur la route conversationnelle canonique, mais arrêtent d encourager la verbalisation du protocole interne. Le plancher `BALANCED` reste substantiel sans rhétorique, les profils `DEVELOPED` et `DEEP` gardent leur profondeur tout en privilégiant une expression plus humaine et naturelle, et le mode assistant de secours décourage désormais l autoprésentation, la narration des phases et les confirmations inutiles quand l action est déjà réalisable.

> 2026-04-30 — Chat anti prompt-theater runtime truth: la correction ne repose plus seulement sur les prompts. `src/services/ai/chatEngine.ts` fait maintenant passer la réponse générée par une garde `postProcess()` alimentée par le message utilisateur validé. Cette couche détecte les sorties manifestement procédurales, retire les paragraphes de théâtre de prompt, préserve la partie livrable quand elle existe déjà, et transforme les réponses créatives purement méta en clarification brève orientée résultat. Les tests `src/__tests__/chatEngine.test.ts` couvrent désormais stripping du préambule, fallback créatif answer-first et préservation des réponses déjà naturelles.
...
> 2026-05-12 — Display xrandr truth: `src-tauri/src/commands/display_system_commands.rs` remplace les 3 stubs TODO par une implémentation xrandr réelle. `parse_xrandr_environment()` parse `xrandr --query` (résolution, refresh_rate trim `*+`, monitor_count, quality). `parse_xrandr_monitors()` liste les moniteurs connectés. `display_set_environment()` applique gamma brightness via `xrandr --output --brightness`. 9 tests Rust PASS. IPC câblés dans `main.rs`: `display_get_environment`, `display_list_monitors`, `display_set_environment`. AutoHeal: `AH-v90-DISPLAY-STUBS-XRANDR-2026-05-12`.
...
> 2026-05-14 — A11Y memory/governance zero-blocking truth: la tranche finale de réduction WCAG AA ferme les trois derniers blocages canoniques sur `/memory` et `/governance-center` sans ouvrir de nouvelle surface. `src/components/sections/MemorySection.tsx`, `src/components/chat/MemoryDashboard.tsx`, `src/features/memory/MemoryTreeViewer.tsx` et `src/features/memory/MemorySearchPanel.tsx` publient désormais des noms accessibles stables sur tous les combobox réellement rendus et rendent la région scrollable `memory-results` atteignable au clavier; `src/features/governance-center/GovernanceCenterPage.tsx` relève en parallèle le contraste de l action refresh. La preuve associée passe par des Vitest ciblés sur ces composants et par `e2e/a11y/wcag-aa-core.spec.ts`, qui publie maintenant `memory=0`, `governance-center=0` et `aggregate blocking=0 baseline=30`.
...
| `data/knowledge_base/default/creativite_apprentissage_cerveau.json` | KB JSON | v31.5.7 | 7 | 49 |
| `data/knowledge_base/default/relations_amoureuses_attachment_couples.json` | KB JSON | v31.5.8 | 7 | 50 |
| `data/knowledge_base/default/systeme_nerveux_neuroanatomie.json` | KB JSON | v31.5.9 | 7 | 45 |
| `data/knowledge_base/default/systeme_lymphatique_sanguin.json` | KB JSON | v31.5.10 | 7 | 54 |
| `data/knowledge_base/default/psychologie_positive_bien_etre.json` | KB JSON | v31.5.11 | 7 | 55 |
| `data/knowledge_base/default/cognition_sociale_biais_cognitifs.json` | KB JSON | v31.5.12 | 7 | 49 |
| `data/knowledge_base/default/spiritualite_sens_existentiel.json` | KB JSON | v31.6.1 | 7 | 52 |
| `data/knowledge_base/default/neurosciences_cliniques_avancees.json` | KB JSON | v31.6.2 | 7 | 45 |
| `data/knowledge_base/default/systeme_cardiovasculaire_avance.json` | KB JSON | v31.7.1 | 7 | 86 |
| `data/knowledge_base/default/immunologie_auto_immunite.json` | KB JSON | v31.7.2 | 7 | 120 |
| `data/knowledge_base/default/pharmacologie_clinique_avancee.json` | KB JSON | v31.7.3 | 7 | 70 |
| `data/knowledge_base/default/gastroenterologie_hepatologie.json` | KB JSON | v31.7.4 | 7 | 65 |
| `src-tauri/src/knowledge_base_default.rs` | Rust | +30 consts +30 SOURCES, baseline >=231 | — | — |
| `src/services/knowledge_manager/index.ts` | Service Ring 3 | getKnowledgeManagerAgentStatus() | — | — |
| `src/services/knowledge_manager/knowledgeOrganizer.ts` | Service Ring 3 | clusterByTheme, scoreRelevance, detectKnowledgeGaps, buildEnrichedIndex, getKBStats | — | — |
| `src/services/knowledge_manager/memoryOptimizer.ts` | Service Ring 3 | estimateTokens, pinCriticalMemories, compactLTMContext, prioritizeForQuery, optimizeContextWindow, computeMemoryHealthScore | — | — |
| `src/services/research_enricher/index.ts` | Service Ring 3 | getResearchEnricherAgentStatus() | — | — |
| `src/services/research_enricher/chatResearchDetector.ts` | Service Ring 3 | isResearchQuery, detectResearchIntent, extractSearchTerms, buildResearchPrompt | — | — |
| `src/services/research_enricher/kbEnricher.ts` | Service Ring 3 | addRuntimeEnrichment, getRuntimeEnrichments, clearOldEnrichments, enrichKBFromResearch, mergeWithKBContext | — | — |
| `src/services/agents/advancedAgentCatalog.ts` | Catalog | +2 entries knowledge_manager+research_enricher | — | — |
| `src/__tests__/services/ai/kb.communicationSante.test.ts` | Vitest | 152 PASS (p37) | — | — |
| `src/__tests__/services/ai/kb.creativiteRelationsAmoureuses.test.ts` | Vitest | 63 PASS (p38) | — | — |
| `src/__tests__/services/ai/kb.systemeNerveuxLymphatique.test.ts` | Vitest | 62 PASS (p39) | — | — |
| `src/__tests__/services/ai/kb.psychologiePositiveBiais.test.ts` | Vitest | 97 PASS (p40) | — | — |
| `src/__tests__/services/ai/kb.spiritualiteNeuroClinique.test.ts` | Vitest | 78 PASS (p41) | — | — |
| `src/__tests__/services/ai/kb.cardiovasculaireImmunologie.test.ts` | Vitest | 92 PASS (p42) | — | — |
| `src/__tests__/services/ai/kb.pharmacologieGastro.test.ts` | Vitest | 59 PASS (p43) | — | — |
| `src/__tests__/services/knowledge_manager/knowledgeManager.test.ts` | Vitest | 32 PASS (knowledge_manager) | — | — |
| `src/__tests__/services/research_enricher/researchEnricher.test.ts` | Vitest | 30 PASS (research_enricher) | — | — |

Corpus clinique: profils toxiques (p24) → protection (p25) → traumatologie/attachement (p26) → dependance/therapies (p27) → CNV + deuil (p28) → TCC + sante mentale/resilience (p29) → neurodiversite (ADHD/TSA/HPI) + DBT avance (p30) → humaniste/existentiel + psychodynamique (p31) → addiction/EM + intimite/sexualite/EFT (p32) → developpement enfant/parentalite + psychosomatique corps-esprit (p33) → psychiatrie clinique diagnostics + neuropsychologie memoire cerveau (p34) → philosophie ethique existentielle + relations humaines groupes sociaux (p35) → sociologie/economie/politique + neurosciences/emotions/decision (p36) → communication/leadership/management + psychologie sante/comportements (p37) → creativite/apprentissage/cerveau + relations amoureuses/attachement adulte/couples (p38) → systeme nerveux/neuroanatomie/neurotransmetteurs/pathologies + systeme lymphatique/sang/immunite/hematologie (p39) → psychologie positive/PERMA/forces VIA/gratitude/flow/résilience/mindfulness + cognition sociale/biais cognitifs/heuristiques/ToM/influence (p40) → spiritualité/logothérapie/Jung/Maslow/stoïcisme/transpersonnel/mort + sommeil/chronobiologie/céphalées/vertiges/douleur neuropathique/neuromodulation (p41) → cardiologie clinique/HTA/SCA/IC/arythmies/valvulopathies/biomarqueurs + immunologie/auto-immunité/biothérapies/allergie/immuno-oncologie (p42) → pharmacologie clinique ADME/CYP450/antibiotiques/psychotropes/analgésiques/pharmacovigilance/urgences + gastroentérologie/hépatologie MICI/cirrhose/hépatites/pancréas/endoscopie/cancer digestif (p43) — agents Ring 3: knowledge_manager + research_enricher
...
## [2026-05-01] V32.0.0 — MetaEnergy Module + SingularityMonitor MetaEnergy Panel
...
> 2026-05-02 — Default conversation depth + Ollama preference truth: `src/services/ai/responsePolicy.ts` requalifie les modes `default` et `standard` sur `BALANCED` pour le parcours conversationnel quotidien, ce qui retire le biais "DEVELOPED par défaut" sur les demandes ordinaires. Dans le même lot, `RESPONSE_PROFILES.DEVELOPED` et `DEEP` passent `ollama` en premier provider quotidien, `src/services/ai/providers/ollama.ts` assouplit le prompt fallback en assistant conversationnel naturel, et `src/config/chatModes.config.ts`/`src/services/ai/chatModes{.data,.ts}` réalignent le prompt `default` vers une réponse proportionnée plutôt que systématiquement longue. Preuves Rule 16: Vitest `responsePolicy.unit`, `chatDefaultInstructions`, `chatModes.phase17`, `chatModes.config`, `chatModes.runtimeDepth`, `useChat-streaming`, `online-availability` PASS; gate `verify:ollama:cline` PASS. AutoHeal: AH-2026-05-02-CONVERSATION-DEFAULT-OLLAMA-ALIGN-0001.

> 2026-05-02 — Authority-boundary correction truth: la posture `Copilote de Cohérence` demandée par l utilisateur ne doit pas piloter TITANE runtime directement. Le lot retire cette posture des prompts produit (`src/config/chatModes.config.ts`, `src/services/ai/chatModes.data.ts`, `src/services/ai/chatModes.ts`, `src/services/ai/providers/ollama.ts`) et la reroute vers l environnement de développement Cline/VS Code (`.clinerules/00-kernel.md`, `.clinerules/hooks/TaskStart`, `.cline/config-optimized.sh`). Le mode `default` retrouve `memoryScope: 'session'`. Preuves: `chatDefaultInstructions`, `chatModes.config`, `verify:ollama:cline`, `verify_instructions`, `detect_recurrence`, `pnpm run check`. AutoHeal: AH-2026-05-02-CLINE-RUNTIME-AUTHORITY-BOUNDARY-0001.

> 2026-05-02 — Active chat Ollama truth correction: le canon Ollama `gemma2:2b` était déjà qualifié sur les surfaces de config, mais pas sur deux surfaces de chat encore actives. `src/services/ai/ConversationManager.ts` publie désormais `DEFAULT_OLLAMA_MODEL` pour la voie `local -> ollama`, et `src/hooks/useChat.ts` recommande maintenant `ollama pull gemma2:2b` dans la recovery UI `Configuration IA Requise`. Preuves Rule 16: `src/__tests__/omega/conversation-manager.test.ts`, `src/hooks/__tests__/useChat.fallbackMessage.test.ts`, plus `verify:ollama:cline` et `pnpm run check`. AutoHeal: AH-2026-05-02-CHAT-OLLAMA-ACTIVE-TRUTH-0001.

> 2026-05-02 — Q&A Advanced Simulation Suite + Fine-tuning chatModes: Création de 4 nouveaux artefacts de test couvrant les simulations Q&A avancées du chat TITANE∞. (1) `e2e/desktop/chat-qa-mode-validation.wdio.test.cjs` : suite WDIO Tauri natif 10 modes × 10 scénarios = 100 Q&A réels, scoring NLP 3 axes (longueur/mots-clés/français), génération automatique de suggestions fine-tuning. (2) `e2e/critical/chat-qa-all-modes.spec.ts` : suite Playwright structurelle 23 modes, 2 Q&A/mode, mode switching, runtime state coherence. (3) `src/__tests__/services/ai/chatModes-full-coverage.test.ts` : 62 tests Vitest exhaustifs — schema integrity 23 modes, bornes température [0.25,1.0] / maxTokens [500,16000], utilitaires validateModeId/getModeConfig/isModeAllowed/getAccessibleModes, ACTIVE_MODE_IDS, MODES_BY_CATEGORY, cohérence cross-modes. 62/62 PASS. (4) `src/__tests__/services/cognitive/ConversationEvaluationEngine.test.ts` : 37 tests Vitest — 9 métriques [0,1], cycle addTestScenario/runTestScenario, runAllTests, generateReport, events, fallback offline. 37/37 PASS. Fine-tuning: `src/services/ai/chatModes.data.ts` mode default reçoit un plancher de profondeur explicite (min 3 phrases substantielles) corrigeant récurrence AH-2026-04-26. AutoHeal: AH-2026-05-02-CHAT-QA-MODE-VALIDATION-0001, AH-2026-05-02-CHAT-QA-PLAYWRIGHT-MODES-0002, AH-2026-05-02-CHAT-MODES-VITEST-COVERAGE-0003, AH-2026-05-02-CONVERSATION-EVAL-ENGINE-TEST-0004, AH-2026-05-02-CHATMODES-FINETUNE-CORRECTIONS-0005.

> 2026-05-02 — Provider recovery message truth alignment (ollama lane): `src/hooks/useConversationEngine.ts` remplace le bloc statique d actions de recuperation provider par une construction contextuelle en fonction du provider demande. Pour `ollama|local`, la surface conversation publie des actions runtime locales explicites (`http://127.0.0.1:11434/api/tags`, verification modele local attendu, redemarrage Ollama) tout en conservant la regle no silent fallback et la selection UI d origine. Couverture Rule 16: `src/__tests__/hooks/useConversationEngine.test.ts` valide la presence des actions Ollama et l absence de guidance cloud generique sur ce lane.
...
## v35.1.5 (2026-05-14) — TimePage normalise les snapshots browser degradés
...
## v35.1.5 (2026-05-14) — HyperCenter ferme un drift allowlist de surface active

> `src/components/HyperCenter/HyperCenter.tsx` utilisait deja la voie canonique `tauriClient` pour charger son etat, ses pensees et ses insights, mais `src/lib/security.ts` ne whitelistaient pas encore cette famille Hyper. En browser, le rejet L1 se produisait avant toute degradation runtime legitime et la capture `hyper-center` remontait des erreurs `Command "hyper_get_state" is not in whitelist`, puis `hyper_get_thoughts`.

> La correction reste purement contractuelle et locale a la frontiere frontend: `ALLOWED_COMMANDS` couvre maintenant toute la famille active Hyper Center (`hyper_init`, `hyper_get_state`, `hyper_get_thoughts`, `hyper_get_insights`, `hyper_set_mode`, `hyper_think`, `hyper_reason`, `hyper_imagine`, `hyper_generate_insight`) et le test anti-regression `allowed-commands-legacy-prune-v34_0_6.test.ts` verrouille ce perimetre. La capture browser `hyper-center` redevient ainsi une preuve de surface runtime reelle, sans faux bruit de whitelist parasite.

## v35.1.5 (2026-05-14) — AppShell et Command Palette ferment un drift de typage transversal

> Le balayage `check + lint` a expose un drift contractuel transversal, non visible dans Playwright: `lazyWithRetry` etait type contre `ComponentType<unknown>`, trop strict pour plusieurs routes lazy reelles (`TitanePage`, `DevPage`, `TotalDevPage`) et pour `AdminPage` exporte via `memo(...)`. En parallele, `ConversationSection` lisait encore `reflection.evaluated`, champ absent du contrat `CognitiveRuntimeTrace`, et le wrapper shadcn `command.tsx` injectait un attribut DOM invalide `cmdk-input-wrapper` qui cassait `eslint`.

> La correction reste minimale et locale: `lazyWithRetry` est detendu sur `ComponentType<any>`, `AdminPage` est normalise via un loader explicite, `ConversationSection` derive le stage `reflection` depuis les vrais signaux runtime, `DevPage` supprime un cast fautif vers `unknown`, et `command.tsx` bascule vers `data-cmdk-input-wrapper` en conservant exactement le meme ciblage CSS. Les preuves `tsc`, `eslint`, `command-palette`, les captures `titane/admin/dev` et l E2E Vitest Tauri repassent toutes vertes apres ce hardening.

## v35.1.5 (2026-05-14) — Reduction a11y partagee sur conversation et dashboard

> Le proof pack historique `proof_packs/v34.0.7-a11y` laissait encore des violations bloquees sur `titane-conversation` et `dashboard`. Un diagnostic runtime Axe cible a isole quatre fautes communes a la meme tranche UI: deux selects sans nom accessible, le toggle `toggle-audio-tts` declare comme switch avec des attributs ARIA invalides, et la `.toast-container` marquee par un `aria-label` sur une semantique de role inadaptée.
...
## 2026-05-17 — Ollama DEV awareness + UI certification
