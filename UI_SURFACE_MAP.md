...
- **Contrat visible** : la surface simulee `/orchestration-intelligence` expose maintenant un `PageHealthBanner` canonique `SIMULATED`; les surfaces desktop capturees disposent d ancres stables `page-time-content`, `page-dev-content`, `page-fusion-content`, `page-twins-content`, `page-optimization-content`.
- **Preuves / gates realignes** : [e2e/desktop/ui-desktop-main-menu-capture-reconciliation.wdio.test.js](e2e/desktop/ui-desktop-main-menu-capture-reconciliation.wdio.test.js) pointe les vrais `data-testid` d onglets (`tab-overview`, `tab-time-*`, `tab-admin-*`, `tab-dev-*`); [e2e/production/ui-production-route-proof.spec.ts](e2e/production/ui-production-route-proof.spec.ts) aligne `/htf` sur `htf-module-page`; le manifest desktop v50 regenere couvre **30 routes**, **22 tabs**, **38 safe actions** et **13 sensitive actions**.
- **Tests** : Vitest pages touchees + gate manifest (`35 + 40 tests PASS`), `pnpm run verify:ui-surface-registry`, `pnpm run verify:ui-desktop-coverage`, `pnpm run verify:ui-production-route-proof`, Playwright `ui-production-route-proof` PASS.
- **Rollback** : `git restore -- src/modules/OrchestrationIntelligenceCenter.tsx src/pages/TimePage.tsx src/pages/DevPage.tsx src/pages/PerfectFusionDashboard.tsx src/pages/TwinsPage.tsx src/pages/UltimateOptimizationDashboard.tsx src/__tests__/pages/DevPage.test.tsx src/__tests__/pages/OrchestrationIntelligenceA11yContrast.test.tsx src/__tests__/pages/PerfectFusionDashboard.test.tsx src/__tests__/pages/TimePage.test.tsx src/__tests__/pages/TwinsPage.test.tsx src/__tests__/pages/UltimateOptimizationDashboard.test.tsx e2e/desktop/ui-desktop-control-inventory.wdio.test.js e2e/desktop/ui-desktop-main-menu-capture-reconciliation.wdio.test.js e2e/desktop/ui-desktop-safe-actions.wdio.test.js e2e/production/ui-production-route-proof.spec.ts scripts/verify/verify-ui-desktop-coverage.mjs scripts/verify/verify-ui-production-route-proof.mjs src/services/agent/__tests__/uiDesktopManifestGate.test.ts docs/ui/desktop/generated/UI_DESKTOP_ROUTE_MANIFEST_v50.json docs/ui/desktop/generated/UI_DESKTOP_CONTROL_INVENTORY_v50.json docs/ui/desktop/generated/UI_DESKTOP_CONTROL_INVENTORY_v50.md docs/ui/desktop/generated/UI_DESKTOP_ACTION_CLASSIFICATION_v50.md docs/ui/desktop/generated/UI_DESKTOP_FRONTEND_BACKEND_MAP_v50.md docs/ui/desktop/generated/UI_DESKTOP_TEST_COVERAGE_MATRIX_v50.md UI_SURFACE_MAP.md docs/CARTOGRAPHY_COMPLETE.md registry/ui-events.jsonl scripts/autoheal/autoheal_rules.jsonl`
...
- Preuves associées: [src/components/sections/**tests**/ConversationSection.modeBridge.test.tsx](/home/titane-os/Documents/GitHub/TITANE_INFINITY/src/components/sections/__tests__/ConversationSection.modeBridge.test.tsx) et [e2e/critical/chat-interaction.spec.ts](/home/titane-os/Documents/GitHub/TITANE_INFINITY/e2e/critical/chat-interaction.spec.ts).
...
- Preuves associées: [src/**tests**/services/ai/chatModeUnifiedRuntimeBridge.test.ts](/home/titane-os/Documents/GitHub/TITANE_INFINITY/src/__tests__/services/ai/chatModeUnifiedRuntimeBridge.test.ts) et [src/**tests**/components/chat/ChatModeSelector.runtimePromptBridge.test.tsx](/home/titane-os/Documents/GitHub/TITANE_INFINITY/src/__tests__/components/chat/ChatModeSelector.runtimePromptBridge.test.tsx).
...
- Garde unitaire composant: [src/**tests**/components/chat/ChatModeSelector.runtimePromptBridge.test.tsx](/home/titane-os/Documents/GitHub/TITANE_INFINITY/src/__tests__/components/chat/ChatModeSelector.runtimePromptBridge.test.tsx) verrouille maintenant explicitement la variante `compact` et son selector stable `chat-mode-selector-select`, qui est la variante réellement montée sur la toolbar conversationnelle canonique.
- Garde unitaire surface canonique: [src/components/sections/**tests**/ConversationSection.modeBridge.test.tsx](/home/titane-os/Documents/GitHub/TITANE_INFINITY/src/components/sections/__tests__/ConversationSection.modeBridge.test.tsx) verrouille aussi désormais les attributs `data-conversation-mode` et `data-chat-store-mode` directement sur `page-conversation`, en plus du bridge `setMode` -> store.
- Garde helper runtime: [src/components/sections/**tests**/ConversationSection.test.ts](/home/titane-os/Documents/GitHub/TITANE_INFINITY/src/components/sections/__tests__/ConversationSection.test.ts) verrouille maintenant aussi les suffixes `Conversation mode` / `Store mode` dans `buildConversationRuntimeSummary()` et les badges `conversation-mode:*` / `chat-store-mode:*` dans `buildConversationRuntimeBadges()`.
...
# [2026-04-30] Conversation natural-expression truth: la surface canonique `/titane?tab=conversation` conserve un haut niveau de profondeur, mais les couches actives `src/services/ai/chatEngine.ts`, `src/services/userPreferencesEngine.ts`, `src/services/ai/chatModes.ts`, `src/config/chatModes.config.ts` et `src/ui/pages/ChatIA/InstructionModeManager.ts` n encouragent plus la mise en scène du protocole interne. TITANE raisonne encore fort en coulisses, mais restitue d abord une réponse plus humaine, vivante, directe et exploitable, sans annoncer ses phases ni détailler sa chaîne de raisonnement sauf demande explicite.

# [2026-04-30] Conversation anti prompt-theater runtime truth: la route canonique `/titane?tab=conversation` ne dépend plus uniquement du prompt pour éviter les réponses procédurales. `src/services/ai/chatEngine.ts` applique maintenant en post-traitement une garde runtime qui détecte les réponses manifestement méta-procédurales, retire les préambules du type `je me nomme TITANE / COLLECTE MAXIMALE / Voulez-vous que je continue`, conserve le livrable quand il existe déjà, et rabat les demandes créatives mal servies vers une clarification concise orientée résultat.
...
# [2026-05-14] A11Y zero-blocking truth for memory and governance-center: les dernières dettes WCAG AA bloquantes des routes canoniques `/memory` et `/governance-center` sont maintenant fermées sur la surface active. `src/components/sections/MemorySection.tsx` nomme `knowledge-topic-filter`, `src/components/chat/MemoryDashboard.tsx` nomme les quatre filtres dashboard, `src/features/memory/MemoryTreeViewer.tsx` nomme `memory-tree-filter`, `src/features/memory/MemorySearchPanel.tsx` nomme ses deux combobox et rend `memory-results` focusable/nommé pour le clavier, et `src/features/governance-center/GovernanceCenterPage.tsx` relève le contraste de l action `Actualiser toutes les données`. La preuve canonique Playwright Axe passe désormais à `memory=0`, `governance-center=0` et `aggregate blocking=0 baseline=30`.
...
## v35.1.5 (2026-05-14) — Fallbacks browser dégradés normalisés sur Dev, Configuration et Optimization

- **`/dev?tab=diagnostics`** : `src/features/system-center/hooks/useSystemDiagnostics.ts` transforme maintenant le fallback browser `tauriProtector` `{ success:false, fallback:true, error }` en diagnostic visible `Degraded` avec résultat `Warning`, au lieu de jeter `Format de diagnostic inattendu`.
- **`/configuration` via Admin Config** : `src/pages/ConfigurationHub.tsx` garde `page-configuration-hub` vivant même quand le snapshot runtime revient avec `runtime: null`, sans `chat_engine`, ou quand `getChatEngineConfig` / `getChatRequestDefaults` retombent en fallback browser sans enveloppe IPC `ok`.
- **`/optimization` et `dev-operations`** : `src/modules/optimization/WebAssemblyCompute.ts` classe désormais l échec de compilation WASM comme fallback JS attendu (`console.warn`) au lieu d un `console.error`, ce qui réaligne la preuve browser avec le comportement réellement dégradé.
- **Preuves ciblées** : `src/features/system-center/hooks/__tests__/useSystemDiagnostics.test.ts`, `src/pages/__tests__/ConfigurationHub.runtimeConfig.test.ts`, `src/__tests__/pages/ConfigurationHub.test.tsx` et `src/modules/optimization/__tests__/WebAssemblyCompute.test.ts` couvrent ces normalisations; la capture Playwright `e2e/critical/ui-prod-capture-v34_0_6.spec.ts` repasse sur `admin-config`, `dev-diagnostics`, `dev-operations` et `optimization` sans nouveau bruit console bloquant.

## v35.1.5 (2026-05-14) — Dev validation tolère les suites QA non-tableau
...
## v35.1.5 (2026-05-14) — AppShell et Command Palette referment leurs derives de typage et de markup

- **Routes lazy `titane/admin/dev/total-dev`** : `src/utils/lazyWithRetry.ts` accepte maintenant correctement les composants React paresseux réellement exportes par les pages (`FC`, `memo`), et `src/App.tsx` normalise le chargeur `AdminPage` sur le meme contrat explicite que les autres routes lazy.
- **Chat / reasoning** : `src/components/sections/ConversationSection.tsx` n interroge plus un champ `reflection.evaluated` absent du contrat `CognitiveRuntimeTrace`; la presence du stage `reflection` est maintenant derivee des signaux runtime reels (`verifierEnabled`, `factualClaimsDetected`, `verified`).
- **`/dev` et command palette** : `src/pages/DevPage.tsx` transmet directement le health store au formatter de truth surface, et `src/components/shadcn/command.tsx` remplace l attribut DOM invalide `cmdk-input-wrapper` par `data-cmdk-input-wrapper`, sans changer le rendu attendu du Command Palette.
- **Preuves cibles** : `pnpm run check`, `pnpm run lint`, `TITANE_E2E_FULL=1 pnpm exec playwright test e2e/critical/command-palette.spec.ts --reporter=line`, `pnpm exec playwright test e2e/critical/ui-prod-capture-v34_0_6.spec.ts --grep 'capture titane|capture admin-config|capture admin-governance|capture dev-diagnostics|capture dev-operations' --reporter=line`, `TITANE_E2E_TAURI=1 pnpm run test:e2e:vitest`.

## v35.1.5 (2026-05-14) — Conversation/dashboard referment une tranche WCAG AA bloquee

- **Selecteurs partages nommes explicitement** : `src/features/chat/ChatProviderSelector.tsx` et `src/components/chat/ChatModeSelector.tsx` donnent maintenant un nom accessible stable a `select-chat-provider` et `chat-mode-selector-select`, ce qui supprime les violations Axe `select-name` qui touchaient en meme temps `titane-conversation` et `dashboard`.
...
- **Surface canonique** : `/titane` et `/titane?tab=conversation` ouvrent maintenant la conversation en layout plein ecran via `data-testid="page-titane"` + `data-layout="chat-fullscreen"`, avec le contenu principal expose par `data-testid="page-titane-content"` et la navigation historique `conversation-history-sidebar`.
- **Surface onglets** : les onglets Titane historiques restent sur les routes explicites non conversation, notamment `/titane?tab=overview`; le tablist standard n est plus rendu sur la surface chat plein ecran.
- **Navigation chat** : `src/components/chat/ConversationHistorySidebar.tsx` centralise la liste, la recherche, la creation et la selection de conversations, puis renvoie vers les sections Titane detaillees sans dupliquer l etat conversation.
- **Hauteur runtime** : `src/components/system/SurfaceRoot.tsx` garantit la chaine `min-h-0/flex-1` pour eviter les scroll hosts casses, tandis que `src/pages/TitanePage.css` isole les styles `titane-chat-fullscreen-layout`.
- **Migration tokens** : les sections Titane touchees (`Conversation`, `Memory`, `MemoryEvolution`, `Transformation`, `Twins`, `Vision`, `SectionLoadingFallback`) consomment les tokens `--titanium-*` au lieu des anciens tokens locaux divergents.
- **Preuves cibles** : `pnpm exec vitest run src/__tests__/pages/TitanePage.tabs.test.tsx src/__tests__/ui/ui-navigation.test.ts src/__tests__/components/chat/MessageBubble.desktopWidth.test.tsx` passe avec 56 tests, puis `pnpm run check`, `pnpm run lint` et `pnpm run verify:frontend-runtime-prebuild` passent avec `BUILD_ALLOWED=YES`.
- **Rollback** : `git revert <commit-titane-chat-fullscreen-sidebar>` ou `git restore -- src/pages/TitanePage.tsx src/pages/TitanePage.css src/components/chat/ConversationHistorySidebar.tsx src/components/sections/ConversationSection.tsx src/components/system/SurfaceRoot.tsx src/components/sections/MemoryEvolutionSection.tsx src/components/sections/MemorySection.tsx src/components/sections/SectionLoadingFallback.tsx src/components/sections/TransformationSection.tsx src/components/sections/TwinsSection.tsx src/components/sections/VisionSection.tsx src/__tests__/pages/TitanePage.tabs.test.tsx src/__tests__/ui/ui-navigation.test.ts UI_SURFACE_MAP.md docs/CARTOGRAPHY_COMPLETE.md registry/ui-events.jsonl scripts/autoheal/autoheal_rules.jsonl`
