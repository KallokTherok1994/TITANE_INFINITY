import { require_jsx_runtime } from './chunk-JO3Y3TZY.js';
import { require_react } from './chunk-65KY755N.js';
import { __commonJS, __export, __toESM } from './chunk-V4OQ3NZ2.js';
...
function steps(numSteps, direction = 'end') {
  return progress2 => {
    progress2 =
      direction === 'end' ? Math.min(progress2, 0.999) : Math.max(progress2, 1e-3);
...
function useVisualElement(
  ...
  const reducedMotionConfig = (0, import_react26.useContext)(
    MotionConfigContext
  ).reducedMotion;
...
function createMotionComponent(
  ...
  const useVisualState2 = isSVGComponent(Component3)
    ? useSVGVisualState
    : useHTMLVisualState;
...
function getProjectionFunctionality(props) {
  ...
  return {
    MeasureLayout:
      drag2?.isEnabled(props) || layout2?.isEnabled(props)
...
function hasScale({ scale: scale2, scaleX: scaleX2, scaleY: scaleY2 }) {
  return (
    !isIdentityScale(scale2) || !isIdentityScale(scaleX2) || !isIdentityScale(scaleY2)
  );
}
...
function axisEqualsRounded(a, b) {
  return (
    Math.round(a.min) === Math.round(b.min) && Math.round(a.max) === Math.round(b.max)
  );
}
...
function useCycle(...items) {
  ...
  const runCycle = (0, import_react41.useCallback)(
    next2 => {
      index.current =
        typeof next2 !== 'number' ? wrap(0, items.length, index.current + 1) : next2;
