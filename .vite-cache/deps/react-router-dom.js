import { require_react_dom } from './chunk-LJXWR6UH.js';
          ...
          async (error, routeId) => {
            ...
            if (renderedStaticContext) {
              ...
              let boundaryRouteId = skipLoaderErrorBubbling
                ...
                : findNearestBoundary(
                    matches,
                    matches.find(m => m.route.id === routeId || m.route.loader)?.route
                      .id || routeId
                  ).route.id;
              let staticContext = {
                matches,
...
function routeHasLoaderOrMiddleware(route) {
  return (
    route.loader != null || (route.middleware != null && route.middleware.length > 0)
  );
}
...
function runClientMiddlewarePipeline(args, handler) {
  ...
  function errorHandler(error, routeId, nextResult) {
    if (nextResult) {
      ...
      let boundaryRouteId = findNearestBoundary(matches, matches[maxBoundaryIdx].route.id)
        .route.id;
      return Promise.resolve({
...
function getTargetMatch(matches, location2) {
  let search =
    typeof location2 === 'string' ? parsePath(location2).search : location2.search;
        ...
        if (!pluginHandled) {
          ...
          if (isArray) {
            for (let i = 0; i < input2.length; i++)
              result +=
                (i ? ',' : '') + (i in input2 ? flatten.call(this, input2[i]) : HOLE);
            str[index2] = `${result}]`;
...
function useFogOFWarDiscovery(
  ...
  React7.useEffect(() => {
    ...
    function registerElement(el) {
      let path =
        el.tagName === 'FORM' ? el.getAttribute('action') : el.getAttribute('href');
      if (!path) {
        return;
      }
      let pathname =
...
function getDocumentHeadersImpl(context, getRouteHeadersFn, _defaultHeaders) {
  let boundaryIdx = context.errors
    ...
    : -1;
  let matches =
    boundaryIdx >= 0 ? context.matches.slice(0, boundaryIdx + 1) : context.matches;
  let errorHeaders;
  if (boundaryIdx >= 0) {
...
function generateSingleFetchRedirectResponse(
  ...
  return generateSingleFetchResponse(request, build, serverMode, {
    result:
      request.method === 'GET' ? { [SingleFetchRedirectSymbol]: redirect2 } : redirect2,
    headers,
    status: SINGLE_FETCH_REDIRECT_STATUS,
  });
}
