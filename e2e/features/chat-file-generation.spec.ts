...
test.describe('Feature: Chat Generated Files Panel', () => {
  ...
  test.beforeEach(async ({ page }) => {
    ...
    const chatNav = page.locator('[data-testid="nav-chat"], [data-section="chat"], a[href*="chat"]').first();
  ...
  test('generated-files-panel appears after triggering a file generation', async ({ page }) => {
    ...
    if (panelExists) {
      ...
      console.warn('[WARN] generated-files-panel did not appear — may be expected if no code block returned or dialog cancelled');
    }
  });
