...
test('Monitoring dashboard — health metrics IPC section présente si données disponibles', async ({ page }) => {
  await page.goto('/');
  const monitoringDashboard = page.getByTestId('monitoring-dashboard');
  ...
  if (isPresent) {
    await expect(page.getByTestId('monitoring-dashboard-health-recurrence')).toContainText('Récurrence incidents');
    await expect(page.getByTestId('monitoring-dashboard-health-ring')).toContainText('Ring le plus impacté');
    await expect(page.getByTestId('monitoring-dashboard-health-leadtime')).toContainText('Lead time moyen');
  }
});
