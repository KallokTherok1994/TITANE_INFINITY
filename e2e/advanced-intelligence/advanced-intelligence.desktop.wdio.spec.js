...
function writeMatrix() {
  try {
    ...
    fs.writeFileSync(
      MATRIX_FILE,
      JSON.stringify({ lock: 'E0', date: '2026-05-06', lanes }, null, 2)
    );
  } catch (_e) {
    /* non-fatal */
  }
...
describe('[AI-DESKTOP-01] Launch + boot intelligence services', () => {
  ...
  it('E0-01-B: App title or root element visible', async () => {
    ...
    const hasTitane =
      title.toLowerCase().includes('titane') ||
...
describe('[AI-DESKTOP-13] Twin consent ledger blocks identity activation', () => {
  it('E0-13: runtime-check hyper-center canonical surface + explicit twin-consent blocker', async () => {
    ...
    const hasModeSelector = await $(
      '[data-testid="hyper-center-mode-selector"]'
    ).isExisting();
...
describe('[AI-DESKTOP-14] Agent effectiveness scorecard accessible', () => {
  it('E0-14-A: Agent scorecard doc is present and readable', async () => {
    ...
    const hasScorecard =
      ...
      content.includes('Scorecard') ||
      content.includes('effectiveness');
    assert.ok(hasScorecard, 'scorecard doc must contain scorecard content');
    recordLane(
      'AI-DESKTOP-14',
      'PASS',
      'AGENT_EFFECTIVENESS_SCORECARD.md present and contains scorecard content'
    );
  });
});
...
describe('[AI-DESKTOP-16] Self-improvement lab requires approval', () => {
  it('E0-16-A: D4 SelfImprovementLabContract — blocksAutoMerge and blocksSelfDeploy constants present', async () => {
    ...
    assert.ok(
      content.includes('auto_merge_blocked: true'),
      'auto_merge_blocked=true in const'
    );
    assert.ok(
      content.includes('self_deploy_blocked: true'),
      'self_deploy_blocked=true in const'
    );
    recordLane(
      'AI-DESKTOP-16',
      'PASS',
...
describe('[AI-DESKTOP-17] AutoHeal recurrence guard', () => {
  it('E0-17-A: autoheal_rules.jsonl exists and has D0–D4 entries', async () => {
    ...
    assert.ok(
      lines.length >= 1671,
      `autoheal must have ≥1671 entries, got ${lines.length}`
    );
    assert.ok(
      content.includes('LOCK_D4_SELF_IMPROVEMENT_LAB_2026_05_06'),
      'D4 entry must be present'
    );
    recordLane(
      'AI-DESKTOP-17',
      'PASS',
      `autoheal entries=${lines.length}, D4 entry confirmed`
    );
  });
  ...
  it('E0-17-B: detect_recurrence.sh governance PASS', async () => {
    ...
    try {
      ...
      recordLane(
        ...
        out
          .split('\n')
          .filter(l => l.includes('PASS') || l.includes('entries='))
          .join('; ')
      );
    } catch (e) {
