...
describe('E0-contract [AI-DESKTOP-13] Twin consent boundary', () => {
  it('C13-1: TwinConsentLedgerContract.ts exists', () => {
    const contractPath = resolve(ROOT, 'src/services/twin_consent_ledger/TwinConsentLedgerContract.ts')
  ...
  it('C13-2: confidence_alone_approves = false in contract', () => {
    const content = readContract('src/services/twin_consent_ledger/TwinConsentLedgerContract.ts')
    expect(content).toMatch(/confidence_alone_approves.*false|confidence.*not.*consent/i)
  })
