...
test.describe('Desktop: MultiProject route proof', () => {
  test('renders the canonical /multiproject route on desktop viewport', async ({
    page,
  }) => {
    ...
    await expect(page.getByTestId('multiproject-agent-status')).toBeVisible({
      timeout: 15000,
    });
    await expect(page.getByTestId('multiproject-project-list')).toBeVisible({
      timeout: 15000,
    });
  });
});
