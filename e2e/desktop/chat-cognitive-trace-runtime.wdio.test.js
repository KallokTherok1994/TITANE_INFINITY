  ...
  it('publishes visible cognitive trace truth in the canonical runtime panel', async function () {
    ...
    const debugInfo = await browser.execute(() => {
      ...
      return {
        ...
        assertantDiv: document
          .querySelector('[data-testid="chat-message-assistant"]')
    ...
    assert.equal(
      compactLocalStorageTruth.hasTrace,
      true,
      'Cognitive trace should exist in localStorage'
    );
    assert.equal(
      compactLocalStorageTruth.verdict,
      'PASS',
      'Verdict should be PASS in localStorage'
    );
    assert.equal(compactLocalStorageTruth.webNeeded, true, 'webNeeded should be true');
    assert.equal(
      compactLocalStorageTruth.memoryInjected,
      true,
      'memoryInjected should be true'
    );
    assert.notEqual(
      compactLocalStorageTruth.qualityScore,
      null,
      'qualityScore should not be null'
    );
