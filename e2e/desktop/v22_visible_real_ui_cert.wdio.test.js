...
describe('V22 — VISIBLE REAL UI CERTIFICATION (AppImage 27.2.0)', () => {
  it('V22-S1 — Boot, surface canonique, MODE decision', async () => {
    ...
    console.log(
      `[V22-S1] DONE => mode=${M.modeDecision} frictions=[${M.frictions.join(', ')}]`
    );
  });
  ...
  it('V22-S2 — Navigation DOM audit + hash nav proof', async () => {
    ...
    try {
      ...
      console.log(
        `[V22-S2] /time nav: ${urlBefore} => ${urlTime} | len: ${lenBefore} => ${lenTime}`
      );
    ...
    console.log(
      `[V22-S2] DONE => tabSwitchWorked=${tabSwitched} tabsCount=${M.tabsCount}`
    );
  });
