...
describe('ONLINE_CHAT_FIX proof driver UI', () => {
  ...
  it('should preserve TIME context through native Tauri conversation_generate without /time', async function () {
    ...
    assert.equal(
      mockFlag,
      false,
      '[R4_NO_MOCK] __TITANE_E2E_CHAT_MOCK__ must be false/absent'
    );
    ...
    assert.ok(
      outcome.runtime.sendTraceState === 'RESPONDED' ||
        outcome.runtime.assistantCount > 0,
    ...
    if (nativeTrace.binding && typeof nativeTrace.binding === 'object') {
      const route =
        nativeTrace.binding.route ||
        nativeTrace.binding.route_path ||
        nativeTrace.binding.routePath;
      const runtimeSource =
      ...
      if (typeof route === 'string' && route.length > 0) {
        assert.ok(
          route.startsWith('/titane'),
          `[R4_BINDING_ROUTE] unexpected route=${route}`
        );
      }
