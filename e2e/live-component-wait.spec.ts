...
test('PATCH-010: Live Provider Routing - Component Wait', async ({ page }) => {
  ...
  const inputMechanisms = await page.evaluate(() => {
    ...
    const messageInputs = document.querySelectorAll(
      '[id*="message"], [placeholder*="message"], [placeholder*="Send"]'
    );
  ...
  console.log(
    '[LIVE-PROVIDER] Input mechanisms found:',
    JSON.stringify(inputMechanisms, null, 2)
  );
  ...
  console.log(
    '[LIVE-PROVIDER] Interactive state:',
    JSON.stringify(isInteractive, null, 2)
  );
