...
test('v34.0.8 all 6 advanced dashboards expose canonical Live indicator contract', async ({
  page,
}) => {
  ...
  await toggle.click();
  await expect(
    page.locator('[data-testid="agent-dashboards-panel-content"]')
  ).toBeVisible({
    timeout: 5_000,
  });
  ...
  for (const { id, prefix } of DASHBOARDS) {
    ...
    await expect(live, `${id}: live container present`).toHaveCount(1, {
      timeout: 5_000,
    });
    ...
    expect(labelText ?? '', `${id}: label contains a HH:MM:SS clock`).toMatch(
      /\d{2}:\d{2}:\d{2}/
    );
    expect(labelText ?? '', `${id}: label contains refresh interval`).toMatch(
      /refresh\s+\d+s/
    );
  }

  await expect(page.locator('[data-testid="monitoring-dashboard"]')).toContainText(
    'Monitoring'
  );
  await expect(
    page.locator('[data-testid="log-analysis-dashboard-service-state"]')
  ).toHaveCount(1);
  await expect(page.locator('[data-testid="log-analysis-dashboard-report"]')).toHaveCount(
    1
  );
...
test('v34.0.8 refresh-now button triggers clock update on a canonical dashboard', async ({
  page,
}) => {
  await page.goto('/dashboard', { waitUntil: 'domcontentloaded', timeout: 30_000 });
  const toggle = page.locator('[data-testid="agent-dashboards-panel-toggle"]');
  ...
  const labelLocator = page.locator(
    '[data-testid="explainability-dashboard-live-label"]'
  );
