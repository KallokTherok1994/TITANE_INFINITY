...
| Runtime extraction | readRuntimeSnapshot in helper | provider/mode/reason/memory-state available | Missing panel but assistant attrs available | Uses panel and assistant attribute fallback | Medium | Preserve dual-source runtime snapshot |
| Multi-turn memory scenario | Test 2 Memory Multi-Turn Integration | Recall includes non-adjacent facts + persistence evidence | Fallback offline with honest explanation | Current verdict: HONEST_OFFLINE_DEGRADED (no ambiguous timeout) | Low | Keep explicit verdict classifier and proof markers |
