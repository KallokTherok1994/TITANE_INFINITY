...
> 2026-04-17 — Security audit bridge desktop runtime truth: le bridge sécurité est désormais exposé par le runtime desktop par défaut du workspace, y compris quand le binaire courant reste en mode `mock`, afin que la voie canonique Tauri puisse persister le journal fédéré et publier l export signé en AppData sans dépendre de la lane `full` actuellement cassée ailleurs. La preuve native WDIO sur `src-tauri/target/debug/titane-infinity` est PASS; la lane `/usr/bin/titane-infinity` reste volontairement BLOCKED jusqu à un build package gouverné séparé avec version bump.
