]633;E;{   echo "# BOOTSTRAP"\x3b   echo "generated_at: $(date -Iseconds)"\x3b   echo "sha: $SHA"\x3b   echo "branch: $(git branch --show-current)"\x3b   echo\x3b   echo "## git status --porcelain"\x3b   echo '```text'\x3b   git status --porcelain || true\x3b   echo '```'\x3b   echo\x3b   echo "## git rev-parse --short HEAD"\x3b   echo '```text'\x3b   git rev-parse --short HEAD || true\x3b   echo '```'\x3b   echo\x3b   echo "## git log -20 --oneline"\x3b   echo '```text'\x3b   git log -20 --oneline || true\x3b   echo '```'\x3b   echo\x3b   echo "## git diff --stat"\x3b   echo '```text'\x3b   git --no-pager diff --stat || true\x3b   echo '```'\x3b   echo\x3b   echo "## tokens (masked)"\x3b   echo '```text'\x3b   if [[ -n "${GO_FOR_PROD_BUILD__TITANE_INFINITY:-}" ]]\x3b then echo 'GO_FOR_PROD_BUILD__TITANE_INFINITY=<present>'\x3b else echo 'GO_FOR_PROD_BUILD__TITANE_INFINITY=<missing>'\x3b fi\x3b   if [[ -n "${GO_FOR_PROD_DEPLOY__TITANE_INFINITY:-}" ]]\x3b then echo 'GO_FOR_PROD_DEPLOY__TITANE_INFINITY=<present>'\x3b else echo 'GO_FOR_PROD_DEPLOY__TITANE_INFINITY=<missing>'\x3b fi\x3b   echo '```'\x3b } > "$PACK/01_BOOTSTRAP.md";0126d5e6-39b5-4996-a459-67a5c79c931c]633;C# BOOTSTRAP
generated_at: 2026-03-02T08:37:35-05:00
sha: 4e17a79e8
branch: MAIN
