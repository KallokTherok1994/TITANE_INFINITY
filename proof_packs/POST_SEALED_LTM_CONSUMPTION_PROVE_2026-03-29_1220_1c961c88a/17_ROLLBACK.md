...
```bash
cd src-tauri && cargo check && cargo test --lib unified_memory && cargo test --test ltm_consumption_proof
```

## Test File Removal

The new test file `src-tauri/tests/ltm_consumption_proof.rs` can be removed if needed:

```bash
rm src-tauri/tests/ltm_consumption_proof.rs