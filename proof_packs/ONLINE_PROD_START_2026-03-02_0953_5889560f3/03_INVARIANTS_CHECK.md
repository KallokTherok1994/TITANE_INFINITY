]633;E;{   echo "# INVARIANTS CHECK"\x3b   echo\x3b   echo "## UI no-web primitives scan"\x3b   echo '```text'\x3b   echo 'pattern: fetch\\(|XMLHttpRequest|WebSocket\\(|EventSource\\(|axios\\.|superagent|node:http|node:https|http://|https://'\x3b   echo 'scope: src/components/** src/pages/** src/App.tsx src/main.tsx src/hooks/**'\x3b   rg -n "fetch\\(|XMLHttpRequest|WebSocket\\(|EventSource\\(|axios\\.|superagent|node:http|node:https|http://|https://" src/components src/pages src/App.tsx src/main.tsx src/hooks || true\x3b   echo '```'\x3b   echo\x3b   echo "## Ring integrity"\x3b   echo '```text'\x3b   pnpm test:architecture\x3b   echo '```'\x3b   echo\x3b   echo "## UI registry append-only"\x3b   echo '```text'\x3b   git --no-pager diff -- registry/ui-events.jsonl | sed -n '1,120p'\x3b   echo '```'\x3b } > "$PACK/03_INVARIANTS_CHECK.md";bfac8617-273c-4c24-a68c-079b62c0a602]633;C# INVARIANTS CHECK

## UI no-web primitives scan
```text
pattern: fetch\(|XMLHttpRequest|WebSocket\(|EventSource\(|axios\.|superagent|node:http|node:https|http://|https://
...
[1m[46m RUN [49m[22m [36mv4.0.18 [39m[90m/home/titane-os/Documents/GitHub/TITANE_INFINITY[39m
 ...
 [32m✓[39m [30m[45m core [49m[39m src/__tests__/architecture/engine-isolation.test.ts [2m([22m[2m3 tests[22m[2m)[22m[32m 24[2mms[22m[39m

[2m Test Files [22m [1m[32m1 passed[39m[22m[90m (1)[39m
[2m      Tests [22m [1m[32m3 passed[39m[22m[90m (3)[39m
[2m   Start at [22m 09:54:12
[2m   Duration [22m 532ms[2m (transform 78ms, setup 205ms, import 9ms, tests 24ms, environment 192ms)[22m

```

## UI registry append-only
```text
```

## ADDENDUM_CANONICAL

- `fetch(` / `XMLHttpRequest` / `WebSocket(` / `EventSource(`: no direct match in scanned UI scope.
- `http(s)://` matches are static literals (links/data URIs), not direct network primitive calls.
