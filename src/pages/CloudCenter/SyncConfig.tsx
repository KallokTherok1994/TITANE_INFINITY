...
const SyncConfig: React.FC<SyncConfigProps> = ({ status, onUpdate }) => {
  ...
  useEffect(() => {
    if (status) {
      setConfig((prev) => ({
        ...prev,
        backend: status.backend === 'LocalFolder' ? 'local_folder' :
                 status.backend === 'S3Private' ? 's3_private' : 'p2p',
        mode: status.sync_mode === 'Manual' ? 'manual' :
              status.sync_mode === 'Auto' ? 'auto' : 'disabled',
      }));
    }
  }, [status]);
      ...
      <div className="config-sections">
        ...
        <section className="config-section">
          ...
          <div className="config-options">
            <label className={`option-card ${config.backend === 'local_folder' ? 'selected' : ''}`}>
              <input
                type="radio"
                name="backend"
                value="local_folder"
                checked={config.backend === 'local_folder'}
                onChange={(e) => setConfig({ ...config, backend: 'local_folder' })}
              />
            ...
            <label className={`option-card ${config.backend === 's3_private' ? 'selected' : ''}`}>
              <input
                type="radio"
                name="backend"
                value="s3_private"
                checked={config.backend === 's3_private'}
                onChange={(e) => setConfig({ ...config, backend: 's3_private' })}
              />
              <div className="option-content">
                <span className="option-icon">☁️</span>
                <span className="option-title">S3 Privé</span>
                <span className="option-desc">
                  MinIO local ou bucket S3 chiffré
                </span>
              </div>
            </label>

            <label className={`option-card ${config.backend === 'p2p' ? 'selected' : ''} disabled`}>
              <input
                type="radio"
                name="backend"
                value="p2p"
                checked={config.backend === 'p2p'}
                onChange={(e) => setConfig({ ...config, backend: 'p2p' })}
                disabled
              />
              <div className="option-content">
                <span className="option-icon">🔗</span>
                ...
                <span className="option-desc">
                  Device-to-device (bientôt disponible)
                </span>
              </div>
            </label>
          </div>
            ...
            <div className="backend-config">
              <div className="form-group">
                ...
                <input
                  ...
                  onChange={(e) => setConfig({ ...config, sync_folder_path: e.target.value })}
          ...
          {config.backend === 's3_private' && (
            <div className="backend-config">
              ...
              <small>
                Les credentials S3 sont stockés dans SecureSecretsEngine
              </small>
            </div>
          )}
        </section>
        ...
        <section className="config-section">
          ...
          <div className="config-options horizontal">
            <label className={`option-pill ${config.mode === 'manual' ? 'selected' : ''}`}>
              <input
                type="radio"
                name="mode"
                value="manual"
                checked={config.mode === 'manual'}
                onChange={(e) => setConfig({ ...config, mode: 'manual' })}
              />
              <span>🖐️ Manuel</span>
            </label>
            ...
            <label className={`option-pill ${config.mode === 'auto' ? 'selected' : ''}`}>
              <input
                ...
                onChange={(e) => setConfig({ ...config, mode: 'auto' })}
              />
              <span>🔄 Automatique</span>
            </label>

            <label className={`option-pill ${config.mode === 'disabled' ? 'selected' : ''}`}>
              <input
                type="radio"
                name="mode"
                value="disabled"
                checked={config.mode === 'disabled'}
                onChange={(e) => setConfig({ ...config, mode: 'disabled' })}
              />
              <span>⛔ Désactivé</span>
            </label>
          </div>
            ...
            <div className="form-group">
              ...
              <div className="input-with-suffix">
                <input
                  ...
                  onChange={(e) =>
                    setConfig({ ...config, auto_sync_interval_secs: parseInt(e.target.value) || 300 })
                  }
                />
                <span className="suffix">secondes</span>
              </div>
              <small>
                Minimum: 60s (1 min) — Maximum: 86400s (24h)
              </small>
            </div>
          )}
        </section>
        ...
        <section className="config-section">
          ...
          <div className="config-options">
            <label className={`option-card small ${config.conflict_resolution === 'last_write_wins' ? 'selected' : ''}`}>
              <input
                type="radio"
                name="conflict"
                value="last_write_wins"
                checked={config.conflict_resolution === 'last_write_wins'}
                onChange={(e) => setConfig({ ...config, conflict_resolution: 'last_write_wins' })}
              />
              <div className="option-content">
                <span className="option-title">⏱️ Dernier écrit gagne</span>
                <span className="option-desc">La version la plus récente est conservée</span>
              </div>
            </label>

            <label className={`option-card small ${config.conflict_resolution === 'kevin_override' ? 'selected' : ''}`}>
              <input
                type="radio"
                name="conflict"
                value="kevin_override"
                checked={config.conflict_resolution === 'kevin_override'}
                onChange={(e) => setConfig({ ...config, conflict_resolution: 'kevin_override' })}
              />
              <div className="option-content">
                <span className="option-title">👤 Préférence Kevin</span>
                <span className="option-desc">La version locale est toujours conservée</span>
              </div>
            </label>

            <label className={`option-card small ${config.conflict_resolution === 'strategic_merge' ? 'selected' : ''}`}>
              <input
                type="radio"
                name="conflict"
                value="strategic_merge"
                checked={config.conflict_resolution === 'strategic_merge'}
                onChange={(e) => setConfig({ ...config, conflict_resolution: 'strategic_merge' })}
              />
              <div className="option-content">
          ...
          <div className="toggle-options">
            ...
            <label className="toggle-option">
              <input
                ...
                onChange={(e) => setConfig({ ...config, limit_during_training: e.target.checked })}
              />
              <span className="toggle-label">
                <span className="toggle-title">Limiter pendant Training Mode</span>
                <span className="toggle-desc">Évite les conflits pendant l'apprentissage</span>
              </span>
            </label>
            ...
            <label className="toggle-option">
              <input
                ...
                onChange={(e) => setConfig({ ...config, limit_during_dev_mode: e.target.checked })}
              />
              <span className="toggle-label">
                <span className="toggle-title">Limiter pendant Developer Mode</span>
                <span className="toggle-desc">Préserve l'état de développement local</span>
              </span>
            </label>
          </div>
        </section>
