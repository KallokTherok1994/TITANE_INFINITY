...
describe('KB: intimite_sexualite_couples_eft (v31.4.6)', () => {
  ...
  it('4 Cavaliers de Gottman mentionnés', () => {
    ...
    const hasCavaliers =
      ...
      (full.toLowerCase().includes('critique') && full.toLowerCase().includes('mepris'));
  ...
  it('mépris (Gottman) mentionné', () => {
    const full = JSON.stringify(kb.sections);
    const hasMepris =
      full.toLowerCase().includes('mepris') || full.toLowerCase().includes('m\u00e9pris');
  ...
  it('cycle réponse sexuelle (Masters Johnson) mentionné', () => {
    const full = JSON.stringify(kb.sections);
    const hasCycle =
      full.toLowerCase().includes('excitation') && full.toLowerCase().includes('orgasme');
    expect(hasCycle).toBe(true);
  });
