...
describe('KB: developpement_enfant_parentalite (v31.4.7)', () => {
  ...
  it("Strange Situation / types d'attachement mentionnés", () => {
    ...
    const hasStrange =
      ...
      (full.toLowerCase().includes('securise') && full.toLowerCase().includes('evitant'));
  ...
  it('Dreikurs ou Janet Nelsen mentionné', () => {
    const full = JSON.stringify(kb.sections);
    const hasDisk =
      full.toLowerCase().includes('dreikurs') || full.toLowerCase().includes('nelsen');
...
describe('KB: psychosomatique_corps_esprit (v31.4.8)', () => {
  ...
  it('section douleur chronique existe', () => {
    const hasDouleur =
      kb.sections.douleur_chronique !== undefined || kb.sections.douleur !== undefined;
    expect(hasDouleur).toBe(true);
  });

  it('section psychoneuro-immunologie existe', () => {
    const hasPNI =
      kb.sections.psychoneuro_immunologie !== undefined || kb.sections.pni !== undefined;
    expect(hasPNI).toBe(true);
  });

  it('section polyvagal Porges existe', () => {
    const hasPV =
      kb.sections.polyvagal_porges !== undefined || kb.sections.polyvagal !== undefined;
    expect(hasPV).toBe(true);
  });

  it('section thérapies somatiques existe', () => {
    const hasSom =
      kb.sections.therapies_somatiques !== undefined || kb.sections.somatic !== undefined;
    expect(hasSom).toBe(true);
  });

  it('section MBSR/mindfulness corps existe', () => {
    const hasMBSR =
      kb.sections.mbsr_mindfulness_corps !== undefined || kb.sections.mbsr !== undefined;
  ...
  it('marqueurs somatiques Iowa Gambling Task mentionné', () => {
    const full = JSON.stringify(kb.sections);
    const hasIowa =
      full.toLowerCase().includes('iowa') || full.toLowerCase().includes('gambling');
  ...
  it('cortisol / axe HPA mentionné', () => {
    const full = JSON.stringify(kb.sections);
    const hasHPA =
      full.toLowerCase().includes('cortisol') || full.toLowerCase().includes('hpa');
  ...
  it('van der Kolk corps trauma mentionné', () => {
    const full = JSON.stringify(kb.sections);
    const hasVdK =
      full.toLowerCase().includes('van der kolk') || full.toLowerCase().includes('kolk');
  ...
  it('MBSR Kabat-Zinn mentionné', () => {
    const full = JSON.stringify(kb.sections);
    const hasKZ =
      full.toLowerCase().includes('kabat') || full.toLowerCase().includes('mbsr');
