...
describe("KB psychologie_positive_bien_etre (v31.5.11)", () => {
  ...
  it("contient 5 piliers PERMA (Positive Emotions, Engagement, Relationships, Meaning, Accomplishment)", () => {
    expect(text(ppb)).toMatch(/Positive Emotions|Engagement|Relationships|Meaning|Accomplishment/);
  });
  ...
  it("contient les 6 vertus VIA", () => {
    expect(text(ppb)).toMatch(/Sagesse|Courage|Humanit[eé]|Justice|Temp[eé]rance|Transcendance/);
  });
  ...
  it("contient les 8 conditions du flow", () => {
    expect(text(ppb)).toMatch(/8 conditions|challenge.{1,30}comp[eé]tence|d[eé]fi.{1,30}comp[eé]tence/);
  });
  ...
  it("contient compassion vs empathie", () => {
    expect(text(ppb)).toMatch(/fatigue compassionnelle|compassion.{1,50}empathie|empathie.{1,50}compassion/);
  });
});
