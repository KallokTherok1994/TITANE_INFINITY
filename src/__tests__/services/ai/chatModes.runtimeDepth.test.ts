...
describe('Chat modes runtime depth floor', () => {
  it('runtime chat modes keep high depth while sounding more human on main conversational lanes', () => {
    ...
    expect(chatModes.default.systemPrompt).toContain('ne pas exposer les phases internes');
  ...
  it('extended chat mode config keeps advanced depth with natural-expression guardrails', () => {
    ...
    expect(CHAT_MODES_CONFIG.standard.systemPrompt).toContain('ne montre ni tes phases ni ton raisonnement interne');
    ...
    expect(CHAT_MODES_CONFIG.reflection.systemPrompt).toContain('restent internes sauf si Kevin demande explicitement la méthode');
  });
});
