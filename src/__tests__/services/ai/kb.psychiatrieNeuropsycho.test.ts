...
const DATA_DIR = join(__dirname, '../../../../data/knowledge_base/default');
...
describe('KB neuropsychologie_memoire_cerveau v31.5.0', () => {
  ...
  it('section systemes_mnésiques_tulving_baddeley exists', () => {
    ...
    const key = Object.keys(sections).find(
      k => k.includes('siques') || k.includes('tulving') || k.includes('baddeley')
    );
  ...
  it('section patient_HM_hippocampe exists', () => {
    const sections = kb.sections as Record<string, unknown>;
    const key = Object.keys(sections).find(
      k => k.includes('HM') || k.includes('hippocampe') || k.includes('patient')
    );
    expect(key).toBeDefined();
  });
  ...
  it('section alzheimer_dementias exists', () => {
    const sections = kb.sections as Record<string, unknown>;
    const key = Object.keys(sections).find(
      k => k.includes('alzheimer') || k.includes('dementia') || k.includes('dement')
    );
    expect(key).toBeDefined();
  });
