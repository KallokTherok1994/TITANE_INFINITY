...
const KB_PATH = join(process.cwd(), 'data/knowledge_base/default');
...
describe('KB therapies_cognitives_comportementales (v31.3.9)', () => {
  ...
  it('retrieval_triggers includes Aaron Beck', () => {
    ...
    expect(
      triggers.some(
        t => t.toLowerCase().includes('aaron beck') || t.toLowerCase().includes('beck')
      )
    ).toBe(true);
  });
  ...
  it('retrieval_triggers includes distorsions cognitives', () => {
    const triggers = kb.retrieval_triggers as string[];
    expect(triggers.some(t => t.toLowerCase().includes('distorsions cognitives'))).toBe(
      true
    );
  });
  ...
  it('retrieval_triggers includes ACT', () => {
    ...
    expect(
      triggers.some(
        t =>
          t.toLowerCase().includes('act') ||
          t.toLowerCase().includes('acceptance and commitment')
      )
    ).toBe(true);
  });
...
describe('KB sante_mentale_prevention_resilience (v31.3.10)', () => {
  ...
  it('retrieval_triggers includes sante mentale', () => {
    ...
    expect(
      triggers.some(
        t => t.toLowerCase().includes('sant') && t.toLowerCase().includes('mentale')
      )
    ).toBe(true);
  });
  ...
  it('Bonanno 4 trajectoires définies', () => {
    const trajectoires = kb.sections.resilience?.Bonanno_trajectoires?.['4_trajectoires'];
