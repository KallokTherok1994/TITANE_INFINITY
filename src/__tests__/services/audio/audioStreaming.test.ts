...
describe('AudioStreamingService', () => {
  ...
  it('startStreaming() devrait démarrer, définir sessionId/isStreaming, et notifier les listeners sur changement d’état', async () => {
    ...
    // Tick 2: Listening -> Listening (no notify)
    await vi.advanceTimersByTimeAsync(200);
    await Promise.resolve();
    ...
    expect(onState).toHaveBeenNthCalledWith(1, 'Listening');
    expect(onState).toHaveBeenNthCalledWith(2, 'Recording');
  ...
  it('getState() devrait retourner Idle sur erreur', async () => {
    ...
    secureInvokeMock.mockImplementation((command: string) => {
      // Erreur non-retriable (évite le backoff + timeouts sous fake timers)
      if (command === 'get_streaming_state')
  ...
  it('forceStop() ne devrait pas throw même si la commande échoue', async () => {
    ...
    // Cleanup (forceStop a échoué → on stop proprement pour éviter les fuites d'interval)
