...
export class DataCollectorEngine {
  ...
  private async extractSuperPrompts(): Promise<DatasetEntry[]> {
    ...
    const memories = await MemoryEngine.recall('super prompt', {
      type: 'code',
      limit: 50,
    });
  ...
  private async extractAIInteractions(): Promise<DatasetEntry[]> {
    ...
    const chatMemories = await MemoryEngine.recall('', {
      type: 'interaction',
      limit: 100,
    });
  ...
  private async extractDevCorrections(): Promise<DatasetEntry[]> {
    ...
    const healMemories = await MemoryEngine.recall('self-healing', {
      type: 'code',
      limit: 30,
    });

    for (const memory of healMemories) {
      entries.push({
