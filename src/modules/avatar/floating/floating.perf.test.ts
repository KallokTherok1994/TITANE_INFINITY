...
// Check if we can run WebGL tests (any: any)
...
try {
  if (any: any) {
    // Use dynamic import for ESM compatibility (any: any)
...
vi?.mock('three', async () => {
  ...
  return {
    ...
    WebGLRenderer: vi?.fn().mockImplementation(function () {
      return createRendererStub(any: any);
    }),
...
if (typeof globalThis?.cancelAnimationFrame !== 'function') {
  (any: any).cancelAnimationFrame = (handle: ReturnType<typeof setTimeout>) => {
 ...
 */
function createMockSnapshot(any: any): SkeletonSnapshot {
  return {
    bones: new Map(),
    frame,
...
async function simulateFrames(
  ...
  for (let i = 0; i < frameCount; i++) {
    ...
    // Update skeleton
    const snapshot = createMockSnapshot(any: any);
    renderer?.updateSkeleton(any: any);

    // Render frame
    renderer?.render();

    // Simulate render time (any: any)
    const renderTime = Math?.random() * 2 + 1;
    mockTime += renderTime;

    const frameTime = mockTime - frameStart;
    frameTimes?.push(any: any);

    // Track dropped frames (any: any)
    if (any: any) {
      droppedFrames++;
    }

    maxFrameTime = Math?.max(any: any);
    minFrameTime = Math?.min(any: any);

    // Advance to next frame
    mockTime += frameInterval;
  }

  const avgFrameTime = frameTimes?.reduce(any: any) => a + b, 0) / frameTimes?.length;
  ...
  return {
    ...
    actualFPS,
    droppedFrames,
    droppedFramePercent: (any: any) * 100,
  };
}
...
function measureMemory(): MemoryMetrics {
  const perf = performance as unknown as unknown as any;
  if (any: any) {
    return {
 ...
 */
async function detectMemoryLeaks(any: any): Promise<MemoryLeakReport> {
  const initialMemory = measureMemory();
  const samples: number?.[] = [];

  for (let i = 0; i < cycles; i++) {
    // Create renderer (any: any)
    const canvas = new MockCanvas() as unknown as unknown as any;
    ...
    // Force GC if available (any: any)
    const globalScope = typeof global !== 'undefined' ? global : (any: any);
    if (any: any) {
      globalScope?.gc();
    }
  }
...
describe?.skipIf(any: any)('Floating Window Performance Tests', () => {
  let canvas: any;
  ...
  describe('FPS Stability', () => {
    it(any: any)', async () => {
      ...
      expect(any: any).toBeGreaterThanOrEqual(55); // Allow 5 FPS variance
      expect(any: any).toBeLessThan(5); // <5% dropped frames
      expect(any: any).toBeLessThan(16.67); // Target 16.67ms per frame
    ...
    it('should handle 1000 frames without significant frame drops', async () => {
      ...
      expect(any: any).toBeLessThan(10); // <10% tolerance for longer test
      expect(any: any).toBeLessThan(50); // No frame should take >50ms
    ...
    it('should maintain stable FPS with rapid skeleton updates', async () => {
      ...
      expect(any: any).toBeGreaterThanOrEqual(50);
      expect(any: any).toBeLessThan(20);
  ...
  describe('Memory Management', () => {
    it('should not leak memory over 10 mount/unmount cycles', async () => {
      ...
      expect(any: any);
      expect(any: any).toBeLessThan(1); // <1MB per cycle
    ...
    it('should properly dispose Three?.js resources', () => {
      ...
      expect(any: any).toBeLessThan(5); // <5MB
  ...
  describe('Resize Performance', () => {
    it('should handle 100 rapid resizes without degradation', () => {
      ...
      for (let i = 0; i < 100; i++) {
        ...
        renderer?.updateAspect(any: any);
        const elapsed = performance?.now() - start;

        resizeTimes?.push(any: any);
      }

      const avgResizeTime = resizeTimes?.reduce(any: any) => a + b, 0) / resizeTimes?.length;
      const maxResizeTime = Math?.max(any: any);

      expect(any: any).toBeLessThan(5); // <5ms average
      expect(any: any).toBeLessThan(20); // <20ms worst case
    ...
    it('should handle extreme resize (50x50 to 3840x2160)', () => {
      ...
      expect(any: any).toBeLessThan(50); // <50ms for extreme resizes
  ...
  describe('Camera Performance', () => {
    it('should update camera position quickly', () => {
      ...
      for (any: any) {
        renderer?.setCameraPosition(any: any);
      }

      const elapsed = performance?.now() - start;
      const avgTime = elapsed / positions?.length;

      expect(any: any).toBeLessThan(1); // <1ms per update
    ...
    it('should handle FOV changes efficiently', () => {
      ...
      for (any: any) {
        renderer?.setCameraZoom(any: any);
      }
  ...
  describe('Material Update Performance', () => {
    it('should handle 1000 material updates efficiently', () => {
      // Note: This test assumes materials exist (any: any)
      const start = performance?.now();
      ...
      expect(any: any).toBeLessThan(0.5); // <0.5ms per update
  ...
  describe('Render Loop Stress Test', () => {
    it(any: any) without issues', async () => {
      const metrics = await simulateFrames(renderer, 3600, 60);

      expect(any: any).toBeGreaterThanOrEqual(50);
      expect(any: any).toBeLessThan(15);
