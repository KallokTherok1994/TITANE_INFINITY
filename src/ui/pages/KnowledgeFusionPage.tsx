...
const KnowledgeFusionPage: React.FC = () => {
  ...
  const getFormatColor = (format: string) => {
    switch (format.toLowerCase()) {
      ...
      case 'plaintext': return 'text-gray-400';
  ...
  const getCategoryColor = (category: string) => {
    switch (category) {
      case 'code': return 'bg-blue-500/20 text-blue-300';
      case 'config': return 'bg-purple-500/20 text-purple-300';
    ...
    <div className="min-h-screen bg-gradient-to-br from-gray-900 via-indigo-900 to-gray-900 p-6">
                ...
                >
                  ...
                  <div className="flex flex-wrap gap-1 mb-2">
                    {doc.categories.slice(0, 3).map((cat, idx) => (
                      <span key={idx} className={`px-2 py-0.5 text-xs rounded ${getCategoryColor(cat)}`}>
                        {cat}
                      </span>
                    ))}
                  </div>
      ...
      <div className="mt-6 bg-gray-800/50 backdrop-blur-xl rounded-2xl p-6 border border-indigo-500/30">
        ...
        <div className="grid grid-cols-2 md:grid-cols-5 gap-3">
          {['PDF', 'DOCX', 'Markdown', 'JSON', 'TXT', 'CSV', 'XML', 'Image', 'Audio', 'Video'].map(format => (
