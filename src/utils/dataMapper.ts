  ...
  return {
    bpm:
      typeof data.bpm === 'number'
        ? data.bpm
        : parseInt(safeValue(data.bpm) as string) || 0,
    vitality:
      typeof data.vitality === 'number'
        ? data.vitality
        : parseInt(safeValue(data.vitality) as string) || 0,
    load:
      typeof data.load === 'number'
        ? data.load
        : parseFloat(safeValue(data.load) as string) || 0,
    ...
    connections:
      typeof data.connections === 'number'
        ...
        : Array.isArray(data.connections)
          ...
          : 0,
    density:
      typeof data.density === 'number'
        ? data.density
        : parseFloat(safeValue(data.network_density || data.density) as string) || 0,
...
export function mapSelfHealData(raw: unknown) {
  ...
  return {
    repairs:
      typeof data.repairs === 'number'
        ? data.repairs
        : parseInt(safeValue(data.repairs || data.total_repairs) as string) || 0,
    success_rate:
      typeof data.success_rate === 'number'
        ? data.success_rate
        : parseFloat(safeValue(data.success_rate) as string) || 0,
...
export function mapAdaptiveData(raw: unknown) {
  ...
  return {
    adjustments:
      typeof data.adjustments === 'number'
        ? data.adjustments
        : parseInt(safeValue(data.adjustments || data.total_adjustments) as string) || 0,
    efficiency:
      typeof data.efficiency === 'number'
        ? data.efficiency
        : parseFloat(safeValue(data.efficiency) as string) || 0,
...
export function mapWatchdogData(raw: unknown) {
  ...
  return {
    tick_misses:
      typeof data.tick_misses === 'number'
        ? data.tick_misses
        : parseInt(safeValue(data.tick_misses) as string) || 0,
    anomalies:
      typeof data.anomalies === 'number'
        ? data.anomalies
...
export function mapHarmoniaData(raw: unknown) {
  ...
  return {
    active_flows:
      typeof data.active_flows === 'number'
        ? data.active_flows
        : parseInt(safeValue(data.active_flows || data.flows) as string) || 0,
    balance_score:
      typeof data.balance_score === 'number'
        ? data.balance_score
        : parseFloat(safeValue(data.balance_score || data.balance) as string) || 0,
...
export function mapSentinelData(raw: unknown) {
  ...
  return {
    integrity_score:
      typeof data.integrity_score === 'number'
        ? data.integrity_score
        : parseFloat(safeValue(data.integrity_score || data.integrity) as string) || 0,
    alerts:
...
export function mapSystemData(raw: unknown) {
  ...
  return {
    ...
    selfheal:
      data.selfheal || data.self_heal
        ? mapSelfHealData(data.selfheal || data.self_heal)
        : null,
    adaptive_engine:
      data.adaptive_engine || data.adaptiveEngine
        ? mapAdaptiveData(data.adaptive_engine || data.adaptiveEngine)
        : null,
