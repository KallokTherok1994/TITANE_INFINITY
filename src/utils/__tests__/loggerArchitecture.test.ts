...
describe('Logger architecture anti-cycle truth', () => {
  it('keeps LogLevel in the shared types module and out of utils/logger', () => {
    const sharedLogLevel = readFileSync(
      resolve(process.cwd(), 'src/types/logLevel.ts'),
      'utf8'
    );
    const runtimeLogger = readFileSync(
      resolve(process.cwd(), 'src/utils/logger.ts'),
      'utf8'
    );
    const runtimeConfig = readFileSync(
      resolve(process.cwd(), 'src/config/logLevelConfig.ts'),
      'utf8'
    );
