...
export class SelfHealingExecutor {
  ...
  public async executePlan(plan: ExecutionPlan): Promise<PlanExecutionResult> {
    ...
    console.warn(`[SelfHealingExecutor] 🚀 Starting plan: ${plan.playbookName}`);
    ...
    try {
      ...
      for (let i = 0; i < plan.actions.length; i++) {
        ...
        if (result.status === 'failed' || result.status === 'timeout') {
          if (plannedAction.action.onFailure === 'abort') {
            console.warn(`[SelfHealingExecutor] ❌ Aborting plan due to failed action`);
      ...
      if (needsRollback && plan.rollbackActions.length > 0) {
        console.warn(`[SelfHealingExecutor] 🔄 Performing rollback...`);
  ...
  public abort(): void {
    if (this.abortController) {
      ...
      console.warn('[SelfHealingExecutor] Execution aborted');
    }
  }
  ...
  public setDryRunMode(enabled: boolean): void {
    ...
    console.warn(`[SelfHealingExecutor] Dry run mode: ${enabled}`);
  }
}
