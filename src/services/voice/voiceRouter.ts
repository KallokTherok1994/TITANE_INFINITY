...
class VoiceRouterService {
  ...
  async processVoiceTurn(
    ...
    // Validation
    if (!transcript.trim()) {
      logger.warn('[VoiceRouter] ⚠️ Empty transcript, aborting turn');
      return {
        success: false,
        transcript,
        error: {
    ...
    try {
      ...
      logger.info('[VoiceRouter] 🤖 Phase 1: Calling AI...');
      ...
      if (useEmotional) {
        logger.info('[VoiceRouter] 🎭 Phase 2: Analyzing emotion...');
        ...
        logger.info('[VoiceRouter] 🔊 Phase 3: Starting Emotional TTS...');
        ...
        logger.info('[VoiceRouter] ✅ Emotional TTS completed');
        ...
        logger.info('[VoiceRouter] 🔊 Phase 2: Starting TTS (no emotion)...');
        ...
        logger.info('[VoiceRouter] ✅ TTS completed');
      ...
      logger.info(`[VoiceRouter] 🎉 Voice turn completed in ${duration}ms`);
      ...
      logger.error('[VoiceRouter] ❌ Voice turn failed:', error);
  ...
  private setState(
    ...
    if (prevState !== state) {
      logger.info(`[VoiceRouter] 🔄 State: ${prevState} → ${state}`);
      callback?.(state);
    }
  }
  ...
  async abort(): Promise<void> {
    logger.info('[VoiceRouter] 🛑 Aborting current voice turn');
   ...
   */
  reset(): void {
    logger.info('[VoiceRouter] 🔄 Resetting router');
