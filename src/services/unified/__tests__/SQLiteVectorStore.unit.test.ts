...
describe('SQLiteVectorStore', () => {
  ...
  const createTestEntry = (overrides: Partial<UnifiedMemoryEntry> = {}): UnifiedMemoryEntry => {
    const now = Date.now();
    return {
  ...
  describe('Statistics', () => {
    ...
    it('should group by importance', async () => {
      ...
      // Check total
      const totalImportance = stats.byImportance.low + stats.byImportance.medium + stats.byImportance.high + stats.byImportance.critical;
