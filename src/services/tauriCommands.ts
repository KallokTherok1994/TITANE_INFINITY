...
/**
 * @deprecated Use `invokeTauriCommandCanonical` instead. This wrapper returns
 * the legacy `CoreResponse<T>` shape (`{ success, data, error }`) rather than
 * the canonical IPC contract `{ ok, content, error }` (Rule 6).
 */
