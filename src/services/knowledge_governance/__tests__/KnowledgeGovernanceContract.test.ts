...
describe('KnowledgeSourceAttributionSchema', () => {
  ...
  it('validates a valid attribution', () => {
    expect(KnowledgeSourceAttributionSchema.safeParse(validAttribution).success).toBe(
      true
    );
  });
...
describe('KnowledgeConfidenceSchema', () => {
  ...
  it('rejects score > 1', () => {
    expect(
      KnowledgeConfidenceSchema.safeParse({ ...validConf, composite_score: 1.5 }).success
    ).toBe(false);
  });

  it('rejects score < 0', () => {
    expect(
      KnowledgeConfidenceSchema.safeParse({ ...validConf, composite_score: -0.1 }).success
    ).toBe(false);
  });

  it('rejects invalid confidence_tier', () => {
    expect(
      KnowledgeConfidenceSchema.safeParse({ ...validConf, confidence_tier: 'excellent' })
        .success
    ).toBe(false);
  });
});
...
describe('GovernedKnowledgeItemSchema', () => {
  ...
  it('rejects item with empty content', () => {
    expect(
      GovernedKnowledgeItemSchema.safeParse({ ...validItem, content: '' }).success
    ).toBe(false);
  });

  it('rejects governance_threshold > 1', () => {
    expect(
      GovernedKnowledgeItemSchema.safeParse({ ...validItem, governance_threshold: 1.5 })
        .success
    ).toBe(false);
  });
});
...
describe('C2-UNIT-01 — KnowledgeItemMetadataSchema validates a valid entry', () => {
  ...
  it('rejects invalid domain', () => {
    expect(
      KnowledgeItemMetadataSchema.safeParse({ ...validMeta, domain: 'alien' }).success
    ).toBe(false);
  });
  ...
  it('accepts all validation_status values', () => {
    ...
    for (const vs of statuses) {
      expect(
        KnowledgeItemMetadataSchema.safeParse({ ...validMeta, validation_status: vs })
          .success
      ).toBe(true);
    }
  });
  ...
  it('rejects confidence > 1', () => {
    expect(
      KnowledgeItemMetadataSchema.safeParse({ ...validMeta, confidence: 1.5 }).success
    ).toBe(false);
  });
