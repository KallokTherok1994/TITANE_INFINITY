...
describe('D5 — Intelligence Seal Contract', () => {
  ...
  describe('ACCEPTABLE_VERDICTS', () => {
    ...
    it('includes DRIFT_FOUND_FIXED', () =>
      expect(ACCEPTABLE_VERDICTS).toContain('DRIFT_FOUND_FIXED'));
    it('has exactly 2 acceptable verdicts', () =>
      expect(ACCEPTABLE_VERDICTS).toHaveLength(2));
  });
  ...
  describe('PROGRAM_LOCK_VERDICTS', () => {
    ...
    it('A0I=DRIFT_FOUND_FIXED', () =>
      expect(PROGRAM_LOCK_VERDICTS.A0I).toBe('DRIFT_FOUND_FIXED'));
  ...
  describe('verifyAllLocks — all CLEAN', () => {
    const allClean = ALL_LOCK_IDS.reduce(
      (acc, id) => ({ ...acc, [id]: 'CLEAN' }),
      {} as Record<LockId, string>
    );
    ...
    it('validates against ProgramVerificationResultSchema', () => {
      expect(() =>
        ProgramVerificationResultSchema.parse(verifyAllLocks(allClean))
      ).not.toThrow();
    });
  });

  describe('verifyAllLocks — one failure', () => {
    const oneFailure = ALL_LOCK_IDS.reduce(
      (acc, id) => ({ ...acc, [id]: id === 'C0' ? 'FAIL' : 'CLEAN' }),
      {} as Record<LockId, string>
    );
  ...
  describe('verifyAllLocks — PROGRAM_LOCK_VERDICTS (current program)', () => {
    it('all_locks_acceptable=true for this program run', () => {
      expect(
        verifyAllLocks(PROGRAM_LOCK_VERDICTS, PROGRAM_COMMIT_SHAS).all_locks_acceptable
      ).toBe(true);
    });
  ...
  describe('buildIntelligenceSealRecord — without T4 approval', () => {
    it('seal_level=provisional when all locks pass but no T4', () => {
      const r = buildIntelligenceSealRecord(
        { seal_id: 'seal-01', t4_approval_recorded: false, seal_notes: 'test' },
        false
      );
    ...
    it('all_locks_passed=true', () => {
      const r = buildIntelligenceSealRecord(
        { seal_id: 'seal-01', t4_approval_recorded: false, seal_notes: 'x' },
        false
      );
  ...
  describe('buildIntelligenceSealRecord — with T4 approval', () => {
    it('seal_level=sealed when T4 approved', () => {
      const r = buildIntelligenceSealRecord(
        { seal_id: 'seal-01', t4_approval_recorded: true, seal_notes: 'test' },
        true
      );
  ...
  describe('getD5IntelligenceSealContract', () => {
    ...
    it('flag_name correct', () => {
      expect(getD5IntelligenceSealContract().flag_name).toBe(
        'TITANE_D5_INTELLIGENCE_SEAL'
      );
    });
    it('program name correct', () => {
      expect(getD5IntelligenceSealContract().program).toBe(
        'TITANE_ADVANCED_INTELLIGENCE_PROGRAM'
      );
    });
    ...
    it('validates against D5IntelligenceSealContractSchema', () => {
      expect(() =>
        D5IntelligenceSealContractSchema.parse(getD5IntelligenceSealContract())
      ).not.toThrow();
    });
  });
});
