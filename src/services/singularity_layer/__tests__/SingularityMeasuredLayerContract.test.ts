...
describe('D2 — Singularity Measured Layer Contract', () => {
  ...
  describe('SingularityEventSchema', () => {
    ...
    it('rejects confidence < 0', () => {
      expect(() =>
        SingularityEventSchema.parse({ ...valid, confidence: -0.1 })
      ).toThrow();
    });
    it('rejects negative detected_at_ms', () => {
      expect(() =>
        SingularityEventSchema.parse({ ...valid, detected_at_ms: -1 })
      ).toThrow();
    });
  ...
  describe('INTENSITY_CONFIDENCE_THRESHOLDS', () => {
    ...
    it('significant=0.7', () =>
      expect(INTENSITY_CONFIDENCE_THRESHOLDS.significant).toBe(0.7));
    it('landmark=0.9', () => expect(INTENSITY_CONFIDENCE_THRESHOLDS.landmark).toBe(0.9));
    it('thresholds are strictly increasing', () => {
      expect(INTENSITY_CONFIDENCE_THRESHOLDS.trace).toBeLessThan(
        INTENSITY_CONFIDENCE_THRESHOLDS.notable
      );
      expect(INTENSITY_CONFIDENCE_THRESHOLDS.notable).toBeLessThan(
        INTENSITY_CONFIDENCE_THRESHOLDS.significant
      );
      expect(INTENSITY_CONFIDENCE_THRESHOLDS.significant).toBeLessThan(
        INTENSITY_CONFIDENCE_THRESHOLDS.landmark
      );
    });
  });
  ...
  describe('classifySingularityIntensity', () => {
    ...
    it('0.69 → notable', () =>
      expect(classifySingularityIntensity(0.69)).toBe('notable'));
    it('0.7 → significant', () =>
      expect(classifySingularityIntensity(0.7)).toBe('significant'));
    it('0.89 → significant', () =>
      expect(classifySingularityIntensity(0.89)).toBe('significant'));
    it('0.9 → landmark', () =>
      expect(classifySingularityIntensity(0.9)).toBe('landmark'));
    it('1.0 → landmark', () =>
      expect(classifySingularityIntensity(1.0)).toBe('landmark'));
  });
  ...
  describe('buildSessionSingularityLedger', () => {
    const makeEvent = (
      intensity: 'trace' | 'notable' | 'significant' | 'landmark'
    ): SingularityEvent => ({
  ...
  describe('getD2SingularityLayerContract', () => {
    ...
    it('t3_flag_name correct', () => {
      expect(getD2SingularityLayerContract().t3_flag_name).toBe(
        'TITANE_D2_SINGULARITY_MEASURED'
      );
    });
    it('event_types_defined=5', () =>
      expect(getD2SingularityLayerContract().event_types_defined).toBe(5));
    it('intensity_levels=4', () =>
      expect(getD2SingularityLayerContract().intensity_levels).toBe(4));
    ...
    it('flag_active reflects env', () => {
      expect(getD2SingularityLayerContract().flag_active).toBe(
        SINGULARITY_D2_MEASUREMENT_FLAG
      );
    });
  });
  ...
  describe('SingularityMeasurementResultSchema', () => {
    it('accepts skipped result', () => {
      expect(() =>
        SingularityMeasurementResultSchema.parse({
          measured: false,
          event: null,
          skipped_reason: 'flag off',
        })
      ).not.toThrow();
    });
...
describe('D2-UNIT — v13 Singularity Measurement Accountability', () => {
  ...
  describe('D2MeasurementModeSchema', () => {
    it('accepts passive', () =>
      expect(() => D2MeasurementModeSchema.parse('passive')).not.toThrow());
    it('accepts shadow', () =>
      expect(() => D2MeasurementModeSchema.parse('shadow')).not.toThrow());
    it('accepts active', () =>
      expect(() => D2MeasurementModeSchema.parse('active')).not.toThrow());
    it('accepts disabled', () =>
      expect(() => D2MeasurementModeSchema.parse('disabled')).not.toThrow());
    it('rejects unknown mode', () =>
      expect(() => D2MeasurementModeSchema.parse('turbo')).toThrow());
  });
