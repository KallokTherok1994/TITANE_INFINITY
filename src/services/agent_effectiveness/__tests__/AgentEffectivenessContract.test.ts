...
describe('computeEffectivenessScore', () => {
  ...
  it('high self-correction penalizes score', () => {
    const withoutCorrections = computeEffectivenessScore(
      true,
      0.9,
      perfectLatency,
      perfectTools,
    ...
    const withCorrections = computeEffectivenessScore(
      true,
      0.9,
      perfectLatency,
      perfectTools,
...
function makeAgent(
  ...
  return {
    ...
    trigger_conditions: [
      'new IPC command added',
      'Ring boundary crossed',
      'One Door bypass detected',
    ],
    ...
    proof_files: [
      'proof_packs/LOCK_C0_PROVIDER_MODEL_INTELLIGENCE_ROUTING_2026-05-06/VERDICT.md',
    ],
    ...
    known_limitations: [
      'Cannot detect indirect Ring violations through generated code',
      'Does not inspect runtime dynamic imports',
    ],
...
describe('D0-UNIT-05 known_limitations are required', () => {
  ...
  it('calculateAgentEffectiveness penalizes missing limitations', () => {
    ...
    const withoutLimitations = calculateAgentEffectiveness({
      ...makeAgent(),
      known_limitations: [],
    });
...
describe('D0-UNIT-08 calculateAgentEffectiveness penalizes missing proof', () => {
  ...
  it('agent with no validators scores < 100', () => {
    expect(
      calculateAgentEffectiveness(makeAgent({ required_validators: [] }))
    ).toBeLessThan(100);
  });
  ...
  it('score is always >= 0', () => {
    ...
    expect(
      calculateAgentEffectiveness({ ...worstCase, known_limitations: [] })
