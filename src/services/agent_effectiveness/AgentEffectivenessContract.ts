...
export const ToolUseEfficiencySchema = z.object({
  ...
  redundant_calls: z
    .number()
    .min(0)
    .describe('Calls that returned same result as prior call'),
  failed_calls: z.number().min(0),
  efficiency_score: z
    .number()
    .min(0)
    .max(1)
    .describe('(total - redundant - failed) / total, or 1 if total=0'),
});
...
export const AgentEffectivenessSnapshotSchema = z.object({
  ...
  self_correction_rate: z
    .number()
    .min(0)
    .describe('corrections / total tasks in window (0..∞)'),
...
const EFFECTIVENESS_WEIGHTS = {
  ...
  latency: 0.15,
  tool_efficiency: 0.1,
  correction_penalty: 0.1, // penalty for high self-correction rate
};
...
export function computeEffectivenessScore(
  ...
  const latencyScore = latencyBudget.within_budget
    ? 1.0
    : Math.max(0, 1 - latencyBudget.overage_ms / latencyBudget.budget_ms);
...
export type D0AgentEffectivenessContract = z.infer<
  typeof D0AgentEffectivenessContractSchema
>;
...
export function getD0AgentEffectivenessContract(): D0AgentEffectivenessContract {
  return {
    ...
    integrations: [
      'B2:IntelligenceObservabilityContract',
      'C2:KnowledgeGovernanceContract',
    ],
  };
}
...
export const AgentEffectivenessScorecardSchema = z.object({
  agent_id: z
    .string()
    .min(1)
    .describe('Unique agent identifier matching .agent.md filename'),
  ...
  trigger_conditions: z
    ...
    .min(1)
    .describe('Conditions that activate this agent'),
  allowed_scope: z
    .array(z.string().min(1))
    .min(1)
    .describe('Paths or domains the agent may touch'),
  forbidden_scope: z
    .array(z.string().min(1))
    .min(1)
    .describe('Paths explicitly off-limits'),
  required_inputs: z.array(z.string()),
  required_outputs: z.array(z.string()),
  required_validators: z
    .array(z.string())
    .describe('Validators that must PASS for agent to be trusted'),
  ...
  blocked_drift_count: z
    .number()
    .int()
    .min(0)
    .describe('How many times scope drift was detected'),
  false_positive_count: z
    .number()
    .int()
    .min(0)
    .describe('Agent flagged valid work as problem'),
  false_negative_count: z.number().int().min(0).describe('Agent missed a real problem'),
  known_limitations: z
    .array(z.string().min(1))
    .min(1)
    .describe('Must have at least one known limitation or explicit justification field'),
  risk_level: AgentRiskLevelSchema,
  rollback_expectation: z
    .string()
    .min(1)
    .describe('How to undo this agent effect if wrong'),
  next_gap: z.string().describe('Most important gap or next action for this agent'),
  status: AgentScorecardStatusSchema,
});
export type AgentEffectivenessScorecard = z.infer<
  typeof AgentEffectivenessScorecardSchema
>;
...
export function buildAgentEffectivenessSummary(
  registry: AgentEffectivenessRegistry
): AgentEffectivenessSummary {
  const agents = registry.agents;
  const verdictCount = (v: AgentScorecardVerdict) =>
    agents.filter(a => a.last_verdict === v).length;
  return {
    ...
    missing_limitations_count: agents.filter(a => a.known_limitations.length === 0)
      .length,
