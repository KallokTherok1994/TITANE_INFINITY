...
const makeEntry = (overrides: Partial<TwinIdentityObservationEntry> = {}): TwinIdentityObservationEntry => ({
...
describe('D3 v13 Sidecar — Schemas', () => {
  it('TwinObservationTypeSchema accepts all 11 types', () => {
    const types = ['identity_fact','preference','value','constraint','symbolic_axis','emotional_pattern','project_context','memory_policy','behavioral_instruction','risk_signal','unknown'] as const
  ...
  it('TwinValidationStatusSchema accepts all 8 statuses', () => {
    const statuses = ['hypothesis','requires_kevin_validation','confirmed','rejected','expired','system_observed','blocked','unknown'] as const
    for (const s of statuses) expect(() => TwinValidationStatusSchema.parse(s)).not.toThrow()
  ...
  it('TwinConsentRiskLevelSchema accepts all 5 levels', () => {
    ...
    for (const l of levels) expect(() => TwinConsentRiskLevelSchema.parse(l)).not.toThrow()
...
describe('D3 v13 Sidecar — Policy Helpers', () => {
  it('isIdentitySensitive=true for identity_fact type', () => {
    expect(isIdentitySensitive(makeEntry({ observation_type: 'identity_fact' }))).toBe(true)
  })
  it('isIdentitySensitive=true for identity_sensitive risk level', () => {
    expect(isIdentitySensitive(makeEntry({ risk_level: 'identity_sensitive' }))).toBe(true)
  })
  it('isIdentitySensitive=false for preference with low risk', () => {
    expect(isIdentitySensitive(makeEntry({ observation_type: 'preference', risk_level: 'low' }))).toBe(false)
  })
  it('requiresKevinValidation=true for identity_fact', () => {
    expect(requiresKevinValidation(makeEntry({ observation_type: 'identity_fact' }))).toBe(true)
  })
  ...
  it('isExpiredObservation=true for past date', () => {
    expect(isExpiredObservation(makeEntry({ expires_at: '2020-01-01T00:00:00Z' }))).toBe(true)
  })
  ...
  it('isRejectedOrBlocked=false for confirmed', () => {
    expect(isRejectedOrBlocked(makeEntry({ validation_status: 'confirmed' }))).toBe(false)
  })
})
...
describe('D3-UNIT Tests — Identity Safety Gates', () => {
  ...
  it('D3-UNIT-02: confidence alone cannot grant consent — hypothesis with high confidence cannot affect identity', () => {
    const entry = makeEntry({ confidence: 0.99, validation_status: 'hypothesis', can_affect_identity: true })
  ...
  it('D3-UNIT-04: canAffectIdentity=false for hypothesis even with can_affect_identity=true', () => {
    expect(canAffectIdentity(makeEntry({ validation_status: 'hypothesis', can_affect_identity: true }))).toBe(false)
  })
  ...
  it('D3-UNIT-05: rejected observation cannot affect behavior, memory, or identity', () => {
    const entry = makeEntry({ validation_status: 'rejected', can_affect_behavior: true, can_affect_memory: true, can_affect_identity: true })
  ...
  it('D3-UNIT-06: expired observation cannot affect behavior, memory, or identity', () => {
    const entry = makeEntry({ expires_at: '2020-01-01T00:00:00Z', can_affect_behavior: true, can_affect_memory: true, can_affect_identity: true })
    expect(canAffectBehavior(entry)).toBe(false)
    expect(canAffectMemory(entry)).toBe(false)
    expect(canAffectIdentity(entry)).toBe(false)
  })
  ...
  it('D3-UNIT-07: symbolic_axis with hypothesis cannot affect identity', () => {
    const entry = makeEntry({ observation_type: 'symbolic_axis', validation_status: 'hypothesis', can_affect_identity: true })
    expect(canAffectIdentity(entry)).toBe(false)
  ...
  it('D3-UNIT-08: behavioral_instruction with requires_kevin_validation cannot affect behavior', () => {
    const entry = makeEntry({ observation_type: 'behavioral_instruction', validation_status: 'requires_kevin_validation', can_affect_behavior: true })
    expect(canAffectBehavior(entry)).toBe(false)
  })
  ...
  it('D3-UNIT-09: system_observed status cannot activate identity behavior', () => {
    const entry = makeEntry({ validation_status: 'system_observed', can_affect_identity: true, can_affect_behavior: true })
    expect(canAffectIdentity(entry)).toBe(false)
    expect(canAffectBehavior(entry)).toBe(false)
  })
