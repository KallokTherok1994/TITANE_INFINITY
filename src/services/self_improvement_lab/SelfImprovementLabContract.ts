...
export function advancePipelineStage(
  ...
  if (!flagActive) {
    return {
      ...
      blocked: true,
      blocking_reason: 'flag=false — T4 scaffold only',
    };
  }
...
export type D4SelfImprovementLabContract = z.infer<
  typeof D4SelfImprovementLabContractSchema
>;
...
export const SelfImprovementHypothesisSchema = z.object({
  ...
  confidence: z
    ...
    .min(0)
    .max(1)
    .describe('Signal strength — NOT a promotion proxy'),
...
export const SelfImprovementPatchProposalSchema = z.object({
  ...
  is_applied: z
    .literal(false)
    .describe('Always false — proposals are artifacts, not applied patches'),
  risk_level: ImprovementRiskLevelSchema,
  requires_approval: z.boolean(),
  approval_status: PromotionStatusSchema,
  created_at: z.string(),
});
export type SelfImprovementPatchProposal = z.infer<
  typeof SelfImprovementPatchProposalSchema
>;
...
export const SelfImprovementSandboxPlanSchema = z.object({
  ...
  can_affect_production: z
    .literal(false)
    .describe('Sandbox must never affect production'),
  planned_at: z.string(),
});
...
export type SelfImprovementEvalSnapshot = z.infer<
  typeof SelfImprovementEvalSnapshotSchema
>;
...
export type SelfImprovementApprovalGate = z.infer<
  typeof SelfImprovementApprovalGateSchema
>;
...
// ── Policy Helpers (v15) ────────────────────────────────────────────────────────
const _IDENTITY_SENSITIVE_RISK: ImprovementRiskLevel[] = [
  'identity_sensitive',
  'restricted',
];
const _RUNTIME_SENSITIVE_RISK: ImprovementRiskLevel[] = [
  'runtime_sensitive',
  'security_sensitive',
  'restricted',
];
const _BLOCKING_STATES: ImprovementState[] = [
  'rejected',
  'expired',
  'blocked',
  'unknown',
...
export function buildSelfImprovementSummary(
  records: SelfImprovementLabRecord[]
  ...
  return {
    ...
    approved: records.filter(
      r => r.approval_gate?.approval_status === 'approved_for_promotion'
    ).length,
    rejected: records.filter(
      r => r.state === 'rejected' || r.approval_gate?.approval_status === 'rejected'
    ).length,
