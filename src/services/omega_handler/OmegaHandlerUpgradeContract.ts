...
export const OmegaMemoryHandlerInputSchema = z.object({
  ...
  flag_active: z.boolean(),
  identity_validated: z.boolean().describe('True only when D2+ identity validation gate has passed'),
})
...
export const OmegaMemoryHandlerOutputSchema = z.object({
  ...
  shadow_used: z.boolean().describe('True when shadow mode was invoked (output not injected into pipeline)'),
  validation_status: z.enum(['PASS', 'FAIL', 'SKIP']),
  known_limits: z.array(z.string()).min(1),
  relevant_items_count: z.number().min(0),
  relevance_score: z.number().min(0).max(1),
  identity_safe: z.boolean().describe('True only when identity_validated=true or identity-sensitive behavior was blocked'),
...
export function getD1SelectedHandlerAdapter(): D1SelectedHandlerAdapter {
  return {
    ...
    rust_surface: 'src-tauri/src/omega/memory_bridge.rs::OmegaMemoryBridge::enrich_context',
  }
}
...
export function validateMemoryHandlerOutput(output: OmegaMemoryHandlerOutput): { passed: boolean; violations: string[] } {
  const violations: string[] = []
  ...
  if (output.known_limits.length === 0) {
    violations.push('D1-MH-V4: known_limits must not be empty — required for transparency')
  }
