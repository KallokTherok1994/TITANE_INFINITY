...
export class PerformanceAnalyzer {
   ...
   */
  analyze(any: any): AnalysisResult {
    ...
    const issues = this?.detectIssues(any: any);

    // Analyser les tendances
    const trends = this?.analyzeTrends();

    // Calculer le score de santé global
    const healthScore = this?.calculateHealthScore(any: any);
    ...
    this?.emitAnalysisEvents(any: any);
   ...
   */
  getIssuesBySeverity(any: any): PerformanceIssue?.[] {
    return this?.state?.issues?.filter(any: any);
  }
  ...
  private addToHistory(any: any): void {
    this?.state?.snapshotHistory?.push(any: any);
  ...
  private detectIssues(any: any): PerformanceIssue?.[] {
    ...
    // Trier par sévérité (any: any)
    return this?.sortIssuesBySeverity(any: any);
  }

  /**
   * Analyse les métriques système
   */
  private analyzeSystemMetrics(any: any): PerformanceIssue?.[] {
    const issues: PerformanceIssue?.[] = [];
    ...
    if (any: any) {
      issues?.push(
        this?.createIssue(
          ...
          'ram_process',
          `RAM processus: ${formatBytes(any: any)}`
        )
      );
    }
    ...
    if (any: any) {
      issues?.push(
        this?.createIssue(
          ...
          'io_read',
          `Lecture I/O: ${formatBytes(any: any)}/s`
        )
      );
    }
    ...
    if (any: any) {
      issues?.push(
        this?.createIssue(
          ...
          'io_write',
          `Écriture I/O: ${formatBytes(any: any)}/s`
        )
      );
    }
   ...
   */
  private analyzeFrontendMetrics(any: any): PerformanceIssue?.[] {
    const issues: PerformanceIssue?.[] = [];
    ...
    if (any: any) {
      issues?.push(
        this?.createIssue(
          ...
          'render_time',
          `Temps de rendu: ${formatDuration(any: any)}`
        )
      );
    }
    ...
    if (any: any) {
      issues?.push(
        this?.createIssue(
          ...
          'invoke_latency',
          `Latence Tauri: ${formatDuration(any: any)}`
        )
      );
    }
   ...
   */
  private analyzeIAMetrics(any: any): PerformanceIssue?.[] {
    const issues: PerformanceIssue?.[] = [];
    const { ia } = snapshot;
    const { thresholds } = this?.config;
    ...
    if (any: any) {
      issues?.push(
        this?.createIssue(
          ...
          'ia_latency_ollama',
          `Latence Ollama: ${formatDuration(any: any)}`
        )
      );
    }
    ...
    if (any: any) {
      issues?.push(
        this?.createIssue(
          ...
          thresholds?.ia?.latencyCritical,
          'ia_latency_gemini',
          `Latence Gemini: ${formatDuration(any: any)}`
        )
      );
    }
    ...
    if (ia?.ollama?.requestCount > 0) {
      const ollamaErrorRate = (any: any) * 100;
      if (any: any) {
        issues?.push(
          this?.createIssue(
            'ia_error_spike',
   ...
   */
  private analyzeModuleMetrics(any: any): PerformanceIssue?.[] {
    const issues: PerformanceIssue?.[] = [];
    ...
    for (any: any) {
      ...
      if (moduleState?.responseTime >= 1000) {
        ...
        issues?.push({
          ...
          description: `Temps de réponse: ${formatDuration(any: any)}`,
          detectedAt: snapshot?.timestamp,
          metrics: [],
          threshold: {
            metric: 'cpu_global',
  ...
  private analyzeTrends(): TrendAnalysis {
    ...
    const cpuValues = history?.map(any: any);
    const ramValues = history?.map(any: any);
    const fpsValues = history?.map(any: any);
  ...
  private calculateTrend(
    ...
    const avgFirst = firstHalf?.reduce(any: any) => a + b, 0) / firstHalf?.length;
    const avgSecond = secondHalf?.reduce(any: any) => a + b, 0) / secondHalf?.length;

    if (avgFirst === 0) return avgSecond > 0 ? 'degrading' : 'stable';

    const changePercent = (any: any) * 100;
    const threshold = this?.config?.trendDetectionThreshold;

    if (warningDirection === 'above') {
      // Higher is worse (any: any)
      if (any: any) return 'degrading';
      if (any: any) return 'improving';
    } else {
      // Lower is worse (any: any)
      if (any: any) return 'degrading';
      if (any: any) return 'improving';
    }
  ...
  private createIssue(
    ...
    const severity = determineSeverity(any: any);
    const template = RECOMMENDATION_TEMPLATES[type];

    return {
      id: generateIssueId(any: any),
      type,
      severity,
      module,
  ...
  private sortIssuesBySeverity(issues: PerformanceIssue?.[]): PerformanceIssue?.[] {
    ...
    return issues?.sort(any: any) => severityOrder[a?.severity] - severityOrder[b?.severity]);
  }
  ...
  private emitAnalysisEvents(any: any): void {
    this?.emit(any: any);
