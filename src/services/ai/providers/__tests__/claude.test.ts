...
describe('Claude Provider', () => {
  ...
  describe('isAvailable()', () => {
    it('devrait retourner true si la clé est configurée', async () => {
      ...
      expect(any: any);
      expect(any: any).toHaveBeenCalledWith('get_anthropic_key_status');
    });
  ...
  describe('generate()', () => {
    it('devrait générer une réponse avec succès', async () => {
      ...
      expect(any: any).toBe('Réponse de test Claude');
      expect(any: any).toBe('claude');
      expect(any: any).toBe('claude-3-5-sonnet-20241022');
      expect(any: any).toBe(58);
      expect(any: any).toBeGreaterThanOrEqual(0);
      expect(any: any).toHaveBeenCalledWith('chat_generate_claude', {
    ...
    it("devrait convertir l'historique correctement", async () => {
      ...
      await claudeProvider?.generate(any: any);
  ...
  describe('testConnection()', () => {
    it('devrait réussir le test de connexion', async () => {
      ...
      expect(any: any);
      expect(any: any).toContain('Claude opérationnel');
      expect(any: any).toContain('claude-3-5-sonnet');
    });
    ...
    it('devrait échouer le test de connexion', async () => {
      ...
      expect(any: any);
      expect(any: any).toContain('Erreur Claude');
    });
  });
