    ...
    >
      <div className="flex items-center justify-between">
        ...
        <button
          ...
          className="rounded border border-titanium-border-default px-3 py-1.5 text-xs text-titanium-text-disabled hover:bg-titanium-bg-elevated"
        >
          📋 Copier
        </button>
      </div>
      ...
      {estimation.clientNom && (
        <p
          data-testid="htf-result-client"
          className="text-sm font-medium text-titanium-text-primary"
        >
      ...
      <div
        ...
        className="rounded-lg bg-titanium-bg-base border border-titanium-border-subtle p-3 text-sm text-titanium-text-secondary"
      >
      ...
      {estimation.items.length > 0 && (
        <div data-testid="htf-result-items">
          <h4 className="text-sm font-semibold text-titanium-text-secondary mb-2">
            Détail
          </h4>
          <table className="w-full text-xs border-collapse">
            <thead>
              <tr className="bg-titanium-bg-elevated">
            ...
            <tbody>
              {estimation.items.map((item, i) => (
                <tr key={i} className="border-t border-titanium-border-subtle">
      ...
      >
        <div className="flex justify-between">
          <span className="text-titanium-text-disabled">Sous-total HT</span>
          <span>{estimation.sousTotal.toFixed(2)} $</span>
        </div>
        <div className="flex justify-between text-titanium-text-disabled text-xs">
          <span>TPS (5%)</span>
        ...
        <p className="text-xs text-titanium-text-disabled pt-1">
        ...
        <div data-testid="htf-result-plan">
          ...
          <ol className="list-decimal list-inside space-y-1">
            {estimation.planMiseEnOeuvre.map(step => (
              <li key={step.ordre} className="text-sm text-titanium-text-disabled">
                <span className="font-medium">{step.titre}</span>
                {step.dureeEstimeeH > 0 && (
                  <span className="text-xs text-titanium-text-tertiary ml-2">
