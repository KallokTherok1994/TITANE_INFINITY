...
export const MessageBubble: React.FC<MessageBubbleProps> = React.memo(
  ({ message }) => {
    ...
    const provider = useMemo(() => {
      const rawProvider =
        message.provider || (message.metadata?.provider as string | undefined);
    ...
    const chunkCount =
      typeof message.metadata?.streamChunks === 'number'
        ? message.metadata?.streamChunks
        : undefined;
    ...
    const promptTokens =
      typeof message.metadata?.promptTokens === 'number'
    ...
    const totalTokens =
      typeof promptTokens === 'number' || typeof completionTokens === 'number'
        ...
        : undefined;
    const mode =
      typeof message.metadata?.mode === 'string' ? message.metadata?.mode : undefined;
      ...
      <div className={`message-bubble ${message.role}`}>
        {(provider ||
          ...
          <div className="message-meta">
            {provider && (
              <span className="message-badge message-badge-provider">⚡ {provider}</span>
            )}
            {mode && (
              <span className="message-badge message-badge-mode">Mode {mode}</span>
            )}
            {isStreaming && (
            ...
            {durationLabel && (
              <span className="message-badge message-badge-duration">
                ⏱ {durationLabel}
              </span>
            )}
            {totalTokens != null && (
            ...
            {totalDurationLabel && (
              <span className="message-badge message-badge-total-duration">
                🕒 Total {totalDurationLabel}
              </span>
            )}
            {loadDurationLabel && (
              <span className="message-badge message-badge-load-duration">
                ⚙️ Chargement {loadDurationLabel}
              </span>
            )}
          </div>
        )}
