...
export const MessageListSimple: React.FC<MessageListSimpleProps> = ({
  ...
  messages?.forEach((msg, i) => {
    console.log(`[MessageListSimple] Message ${i}:`, msg?.role, msg?.content?.substring(0, 50));
  });

  return (
    <div className="message-list-container" style={{
      flex: 1,
      overflowY: 'auto',
      padding: '16px',
      display: 'flex',
      ...
      <div style={{
        ...
        DEBUG: {messages?.length || 0} messages | Loading: {isLoading ? 'OUI' : 'NON'} | Error: {error || 'AUCUNE'}
      </div>
