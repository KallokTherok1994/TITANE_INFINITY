...
import {
  ModeBuilder,
  type CustomMode,
} from "@/components/conversation/ModeBuilder";
...
function extractResearchTopic(input: string): string {
  ...
  const stripped = normalized
    ...
    .replace(
      /(une|un)?\s*recherche\s*(sur|dans)?\s*(internet|le web|web)?\s*/i,
      "",
    )
...
function detectBlockCause(report: ResearchReport): string {
  ...
  if (
    markers.includes("ROBOTS_BLOCKED") ||
    markers.includes("M_ROBOTS_BLOCKED")
  ) {
    return "ROBOTS_BLOCKED";
  }

  if (
    markers.includes("RATE_LIMIT_BLOCKED") ||
    markers.includes("M_RATE_BLOCKED")
  ) {
...
function classifyResearchOutcome(report: ResearchReport): ResearchOutcome {
  ...
  if (
    report.answer.sources_count <= 1 &&
    report.answer.retrieved_passages_count <= 1
  ) {
    return "limited";
  }
...
function buildResearchReply(
  report: ResearchReport,
  outcome: ResearchOutcome,
): string {
  const verdict =
    report.trace.markers.find((m) => m.startsWith("VERDICT_")) ??
    "VERDICT_UNKNOWN";
  ...
  if (outcome === "blocked") {
    ...
    const source =
      citations[0]?.url || "Aucune source exploitable pour cette tentative.";
    return [
...
function deriveResearchProviderMeta(
  ...
  const cacheHit = (report.trace.cache_events || []).some(
    (event) => event.kind === "HIT",
  );
  const duration =
    typeof report.trace.timings?.total === "number"
      ? report.trace.timings.total
      : 0;

  let reasonCode: ReasonCode = "OK";
  if (outcome === "blocked") {
    reasonCode =
      detectBlockCause(report) === "RATE_LIMIT_BLOCKED"
        ? "RATE_LIMIT"
        : "POLICY_BLOCKED";
...
export const ConversationSection: React.FC<ConversationSectionProps> = memo(
  () => {
    ...
    const selectedProviderLabel = useMemo(
      () =>
        AVAILABLE_PROVIDERS.find((p) => p.id === selectedProvider)?.name ??
        selectedProvider,
      [selectedProvider],
    );
    ...
    const filteredMessages = useMemo(() => {
      ...
      if (searchNeedle) {
        result = result.filter((m) =>
          m.content.toLowerCase().includes(searchNeedle),
        );
      }
    ...
    const handleSend = useCallback(async () => {
      ...
      if (shouldHandoffToResearch(messageText)) {
        ...
        thinking.addStep(
          "analysis",
          "Détection recherche web (mode chat intégré)...",
        );
        try {
          ...
          if (outcome === "blocked") {
            errorToast(
              "Recherche bloquée par la gouvernance/robots pour cette cible.",
            );
          } else if (outcome === "limited") {
