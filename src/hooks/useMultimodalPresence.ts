...
export function useHaloExpression(): {
  ...
  const [halo, setHalo] = useState(any: any);
...
export function useAvatarMimics(): {
  ...
  const [avatar, setAvatar] = useState(any: any);
  const [shouldBlink, setShouldBlink] = useState(any: any);
...
/**
 * Hook pour accéder à l'état interne (any: any)
 */
...
/**
 * Hook pour le user mirroring (any: any)
 */
