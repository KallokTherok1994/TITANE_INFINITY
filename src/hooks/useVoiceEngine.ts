...
/**
 * ═══════════════════════════════════════════════════════════════════
 *   TITANE∞ v30.0.0 — UNIFIED VOICE ENGINE HOOK
...
export function useVoiceEngine(
  ...
  useEffect(() => {
    ...
    const checkCapabilities = async () => {
      try {
        ...
        logger.error('[useVoiceEngine] Init error:', err);
      }
    };
  ...
  const handleError = useCallback(
    (error: Error | string, context: string) => {
      ...
      logger.error(`[useVoiceEngine] ${context}:`, message);
  ...
  const startRecordingInternal = useCallback(async () => {
    try {
      ...
      if (status.isRecording) {
        logger.warn('[useVoiceEngine] Already recording, ignoring duplicate call');
        return;
      }
      ...
      logger.info('[useVoiceEngine] ✅ Recording started');
    } catch (err) {
      ...
      if (
        ...
        logger.error('[useVoiceEngine] 🔥 Backend stuck, applying force reset...');
        ...
        try {
          ...
          logger.error('[useVoiceEngine] ❌ Force reset failed:', resetErr);
          handleError(
  ...
  const stopRecordingInternal = useCallback(async (): Promise<string> => {
    try {
      ...
      if (!status.isRecording) {
        logger.warn('[useVoiceEngine] Not recording, returning empty transcript');
        return '';
      }
      ...
      logger.info('[useVoiceEngine] ✅ Recording stopped, transcript:', transcript);
  ...
  const processTurnWithAI = useCallback(
    async (transcript: string) => {
      try {
        logger.info('[useVoiceEngine] 🎙️ Processing turn with VoiceRouter...');
        ...
        const result = await voiceRouter.processVoiceTurn(transcript, chat.sendMessage, {
          ...
          onTTSStart: () => {
            logger.info('[useVoiceEngine] 🔊 TTS started');
          },
          onTTSEnd: () => {
            logger.info('[useVoiceEngine] ✅ TTS completed');
          },
          onError: error => {
            logger.error('[useVoiceEngine] ❌ VoiceRouter error:', error);
            handleError(
        ...
        if (result.success) {
          logger.info(`[useVoiceEngine] 🎉 Voice turn completed in ${result.duration}ms`);
        } else {
          logger.error('[useVoiceEngine] ❌ Voice turn failed:', result.error);
        }
      } catch (error) {
        logger.error('[useVoiceEngine] ❌ processTurnWithAI error:', error);
        handleError(
  ...
  const startTurn = useCallback(async () => {
    ...
    if (status.state !== 'idle') {
      logger.warn('[useVoiceEngine] Cannot start turn: state =', status.state);
      return;
    }

    // ✅ v∞.7 PHASE 9: Only start if explicitly called (no VAD auto)
    logger.info('[useVoiceEngine] Manual start (Chat mode) - no VAD auto');
  ...
  const completeTurn = useCallback(async () => {
    try {
      logger.info('[useVoiceEngine] 📝 Completing turn...');

      // Stop recording et obtenir transcription
      const transcript = await stopRecordingInternal();

      if (!transcript || !transcript.trim()) {
        logger.warn('[useVoiceEngine] Empty transcript, cancelling turn');
        if (mountedRef.current) {
          setStatus(prev => ({ ...prev, state: 'idle' }));
        }
        audioStateMachine.reset();
      ...
      logger.error('[useVoiceEngine] ❌ completeTurn error:', error);
      handleError(
        error instanceof Error ? error : new Error(String(error)),
        'completeTurn'
      );
  ...
  const completeTurnWithText = useCallback(
    async (text: string) => {
      try {
        logger.info('[useVoiceEngine] 📝 Completing turn with text:', text);

        if (!text || !text.trim()) {
          logger.warn('[useVoiceEngine] Empty text, cancelling turn');
          if (mountedRef.current) {
            setStatus(prev => ({ ...prev, state: 'idle' }));
          }
          audioStateMachine.reset();
        ...
        logger.error('[useVoiceEngine] ❌ completeTurnWithText error:', error);
        handleError(
          error instanceof Error ? error : new Error(String(error)),
  ...
  const cancelTurn = useCallback(async () => {
    try {
      logger.info('[useVoiceEngine] 🛑 Cancelling turn, state:', status.state);
      ...
      logger.info('[useVoiceEngine] ✅ Turn cancelled successfully');
    } catch (err) {
      logger.error('[useVoiceEngine] ❌ Cancel error:', err);
      // Force reset anyway
      if (mountedRef.current) {
        setStatus(prev => ({
          ...prev,
  ...
  const startDictation = useCallback(async () => {
    if (status.state !== 'idle') {
      logger.warn('[useVoiceEngine] Cannot start dictation: not idle');
      return;
    }
    ...
    try {
      ...
      await startRecordingInternal();
      logger.info('[useVoiceEngine] Dictation started');
    } catch (err) {
  ...
  const stopDictation = useCallback(async (): Promise<string> => {
    if (!status.isRecording) {
      logger.warn('[useVoiceEngine] No dictation in progress');
      return '';
    }
    ...
    try {
      ...
      logger.info('[useVoiceEngine] Dictation stopped, result:', transcript);
      return transcript;
    } catch (err) {
      return '';
    }
  ...
  const speak = useCallback(
    async (text: string) => {
      if (!text.trim()) {
        logger.warn('[useVoiceEngine] Empty text, skipping TTS');
        return;
      }
  ...
  const stopSpeaking = useCallback(async () => {
    try {
      ...
      logger.error('[useVoiceEngine] Stop speaking error:', err);
    }
  }, []);
  ...
  const activateWakeWord = useCallback(() => {
    logger.info('[useVoiceEngine] 🎙️ Activating wake word mode');
  ...
  const deactivateWakeWord = useCallback(() => {
    logger.info('[useVoiceEngine] 🔇 Deactivating wake word mode');
  ...
  const setPushToTalk = useCallback(() => {
    logger.info('[useVoiceEngine] 🎤 Switching to push-to-talk');
  ...
  const enableFullDuplex = useCallback(async () => {
    logger.info('[useVoiceEngine] 🔄 Enabling full duplex mode');
  ...
  const disableFullDuplex = useCallback(async () => {
    logger.info('[useVoiceEngine] 🔇 Disabling full duplex mode');
  ...
  const interrupt = useCallback(async () => {
    logger.info('[useVoiceEngine] 🚨 User interruption');
    await fullDuplexOrchestrator.interrupt();
  }, []);

  const injectInterruption = useCallback(
    async (text: string) => {
      logger.info('[useVoiceEngine] 💬 Inject interruption:', text);
  ...
  const forceVoiceReset = useCallback(async () => {
    try {
      logger.warn('[useVoiceEngine] 🔥 FORCE RESET VOICE ENGINE');
      ...
      logger.info('[useVoiceEngine] ✅ Voice reset complete');
    } catch (error) {
      logger.error('[useVoiceEngine] ❌ Force reset error:', error);
