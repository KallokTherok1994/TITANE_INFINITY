...
interface UseTwinBehaviorReturn {
  ...
  // Actions
  observeValue: (valueName: string, context?: string, confidence?: number) => Promise<string | null>;
  observeCognitive: (pattern: string, context?: string, confidence?: number) => Promise<string | null>;
  observeStyle: (element: string, context?: string, confidence?: number) => Promise<string | null>;
  observeEmotional: (state: string, context?: string, confidence?: number) => Promise<string | null>;
...
export function useTwinBehavior(): UseTwinBehaviorReturn {
  ...
  const observeValue = useCallback(async (
    valueName: string,
    context?: string,
    confidence = 0.7
  ): Promise<string | null> => {
  ...
  const observeCognitive = useCallback(async (
    pattern: string,
    context?: string,
    confidence = 0.7
  ): Promise<string | null> => {
    ...
    setError(null);
    try {
      const id = await numericTwinService.observeCognitivePattern(pattern, context, confidence);
  ...
  const observeStyle = useCallback(async (
    element: string,
    context?: string,
    confidence = 0.7
  ): Promise<string | null> => {
  ...
  const observeEmotional = useCallback(async (
    state: string,
    context?: string,
    confidence = 0.7
  ): Promise<string | null> => {
