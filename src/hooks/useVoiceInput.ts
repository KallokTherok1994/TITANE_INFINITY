...
export function useVoiceInput(config?: AudioConstraints): UseVoiceInputReturn {
  ...
  const getAudioConstraints = useCallback((): MediaStreamConstraints => {
    ...
    logger.info('Audio constraints:', constraints.audio);
    return constraints;
  }, [config]);
  ...
  const startListening = async () => {
    try {
      ...
      logger.info('Audio track settings:', {
      ...
      if (!settings.echoCancellation) {
        logger.warn(
          'Echo cancellation not supported on this device/browser. ' +
  ...
  useEffect(() => {
    return () => {
      if (recordingIdRef.current) {
        voiceService.cancelRecording().catch((err: unknown) => {
