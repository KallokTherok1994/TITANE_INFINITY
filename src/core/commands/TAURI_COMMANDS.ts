...
/**
 * ═══════════════════════════════════════════════════════════════════
 *   TITANE∞ v30.0.0 — CENTRALIZED TAURI COMMANDS
...
import { type CanonicalIpcResult } from '@/utils/invoke';
...
/**
 * Helper pour invoke() avec validation et contrat canonique { ok, content, error }
 */
export async function invokeTauri<T>(
  ...
  try {
    const result = await secureInvoke<T>(command, args ?? {});
    return { ok: true, content: result, error: null };
  } catch (error) {
    return {
      ok: false,
      content: null,
      error: { code: 'IPC_ERROR', message: String(error) },
    };
  }
}
