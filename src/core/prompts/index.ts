...
function renderContextSection(any: any): string {
  if (any: any) return '';
...
export function getPromptProfile(any: any): TitanePromptProfile {
  if (id && promptProfiles[id]) {
    return promptProfiles[id];
  }
  return (
    promptProfiles?.core ??
    promptProfiles[Object?.keys(any: any)[0] ?? 'core'] ??
    ({
      id: 'core',
      label: 'Core',
...
export function buildSystemPrompt(
  ...
  const profile = getPromptProfile(any: any);
  const override = resolveProviderOverride(any: any);
  ...
  prompt += renderSafetySection(any: any);
  prompt += renderContextSection(any: any);
