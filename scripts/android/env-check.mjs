...
function inspectDeviceReadiness(deviceId) {
  ...
  const deviceProvisioned =
    readProp('ro.setupwizard.mode') === 'DISABLED'
      ? '1'
      : runAdb([
          '-s',
          deviceId,
...
const ndkInstalled = Boolean(
  ndkDir && fs.existsSync(ndkDir) && fs.readdirSync(ndkDir).length > 0
);
const deviceState = adb.found
  ? listConnectedDevices()
  : { available: false, devices: [] };
const deviceReadiness = deviceState.devices.map(inspectDeviceReadiness);
const readyDevices = deviceReadiness
...
if (deviceState.devices.length > 0 && readyDevices.length === 0)
  blockers.push('device-not-boot-ready');
...
console.log(
  `rust_android_targets=${requiredTargets.length - missingTargets.length}/${requiredTargets.length}`
);
console.log(
  `missing_rust_targets=${missingTargets.length > 0 ? missingTargets.join(',') : 'none'}`
);
console.log(`connected_devices=${deviceState.devices.length}`);
console.log(
  `device_ids=${deviceState.devices.length > 0 ? deviceState.devices.join(',') : 'none'}`
);
console.log(`ready_devices=${readyDevices.length}`);
console.log(
  `ready_device_ids=${readyDevices.length > 0 ? readyDevices.join(',') : 'none'}`
);
console.log(
  `device_readiness=${
    deviceReadiness.length > 0
      ? deviceReadiness
          .map(
            device =>
...
if (blockers.length > 0) {
  ...
  if (blockers.includes('no-device-connected')) {
    console.log(
      'next_step=connect an Android device or start an emulator, then rerun `corepack pnpm run android:install:latest`'
    );
  } else if (blockers.includes('device-not-boot-ready')) {
    console.log(
      'next_step=wait until the emulator reports sys.boot_completed=1 and user_setup_complete=1, or provision the clean AVD before rerunning the install'
    );
    ...
    console.log(
      'next_step=install Android SDK command-line tools or export ANDROID_HOME / ANDROID_SDK_ROOT'
    );
    ...
    console.log(
      'next_step=finish the missing Android prerequisites, then rerun the local finishing flow'
    );
  }
