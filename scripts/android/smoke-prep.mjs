...
console.log('4. adb wait-for-device && adb shell getprop sys.boot_completed && adb shell pm path android');
console.log('5. node scripts/android/install-latest-apk.mjs --dry-run   # show the preferred APK');
console.log('6. corepack pnpm run android:install:latest              # install the preferred APK once boot-ready');
console.log(
  '7. Set OLLAMA_BASE_URL in the launch shell before starting Android dev/build commands'
);
console.log(
  '8. adb shell am start -n com.titane.infinity/.MainActivity'
);
console.log(
