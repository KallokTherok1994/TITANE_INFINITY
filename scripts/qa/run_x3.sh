...
for run in 1 2 3; do
  ...
  LOG_FILE="$PACK_DIR/logs/${SAFE_ID}_run${run}.log"

  {
    echo "RUN|ts=$TS|id=$CMD_ID|run=$run|cmd=$CMD_STR|log=$LOG_FILE"
  } >> "$AGG_PATH"
  ...
  {
    echo "RESULT|ts=$TS|id=$CMD_ID|run=$run|exit=$EC|cmd=$CMD_STR|log=$LOG_FILE"
  } >> "$AGG_PATH"

  if [ "$EC" -ne 0 ]; then
    echo "FAIL: $CMD_ID run $run (exit=$EC)" >&2
    exit "$EC"
  fi
done
