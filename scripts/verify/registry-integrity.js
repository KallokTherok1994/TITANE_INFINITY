...
function verifyIntegrity() {
  ...
  assert(
    Number(snapshot.eventCount) === events.length,
    `snapshot.eventCount (${snapshot.eventCount}) != events (${events.length})`
  );
  ...
  for (const e of events) {
    if (e.schemaVersion === 2) {
      ...
      assert(
        typeof e.cycleId === 'string' && e.cycleId.length > 0,
        'event.cycleId manquant'
      );
    }
  }
  ...
  if (activeCycleId) {
    assert(
      cycles.open.some(c => c.id === activeCycleId),
      'cycles.activeCycleId ne pointe pas vers un cycle ouvert'
    );
  }
  assert(
    Array.isArray(cycles.open) && Array.isArray(cycles.closed),
    'cycles.open/closed invalides'
  );
