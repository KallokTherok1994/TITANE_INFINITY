...
rg -q 'chat\.mcp\.enabled' .vscode/settings.json || fail '.vscode/settings.json missing chat.mcp.enabled'
rg -q '"chat.mcp.enabled"\s*:\s*true' .vscode/settings.json || fail '.vscode/settings.json chat.mcp.enabled is not true'
...
pass 'MCP runtime settings and transport wired correctly'
