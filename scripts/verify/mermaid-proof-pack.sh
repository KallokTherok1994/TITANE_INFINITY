...
run_capture "diff_intel" "$PACK_DIR/diff_intel.txt" bash -c 'bash scripts/verify/mermaid-diff-intel.sh'
run_capture "status_report_update" "$PACK_DIR/status_report_update.txt" bash -c 'bash scripts/verify/mermaid-status-report.sh'
