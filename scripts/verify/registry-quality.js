...
import { assert, loadEventsJsonl, looksLikeSecret, resolveRepoRoot } from '../registry/registry-lib.js';
...
function isVague(desc) {
  ...
  const hasSignal = /\.(ts|tsx|js|yml|json|md)\b/i.test(d) || d.includes('/') || /\b(pnpm|vitest|cargo|tauri|workflow|gate|suite)\b/i.test(d) || /\d/.test(d);
  return !hasSignal;
}
...
function verifyQuality() {
  ...
  for (const e of events) {
    ...
    assert(typeof e.description === 'string' && e.description.trim().length >= 20, 'description trop courte');
    assert(!isVague(e.description), 'description vague');
    assert(!looksLikeSecret(e.description), 'secret détecté (description)');
    if (e.meta?.command) assert(!looksLikeSecret(e.meta.command), 'secret détecté (command)');

    assert(['LOW', 'MEDIUM', 'HIGH', 'CRITICAL'].includes(e.severity), 'severity invalide');
    assert(['TESTS', 'CI', 'ARCHITECTURE', 'SECURITY', 'DELIVERY'].includes(e.impact), 'impact invalide');
    ...
    if (e.type !== 'CYCLE_END') {
      assert(Array.isArray(e.next_actions) && e.next_actions.length >= 1, 'next_actions manquant');
    }
    if (Array.isArray(e.next_actions)) {
      for (const a of e.next_actions) {
        assert(typeof a === 'string' && a.trim().length >= 8, 'next_actions item trop court');
    ...
    if (attemptKeys.has(key)) {
      assert(typeof e.meta?.justification === 'string' && e.meta.justification.trim().length >= 15, 'attempt répété sans justification');
    }
