...
function getGitDiff() {
  try {
    ...
    const diff = execSync('git diff --name-only HEAD~1 2>/dev/null || git diff --name-only origin/MAIN', {
      cwd: ROOT_DIR,
      encoding: 'utf8',
    });
...
function verify() {
  ...
  if (!fs.existsSync(EVENTS_FILE)) {
    ...
    console.error('   Run: pnpm registry:log -- --type=REGISTRY_INIT --desc="Initialize registry"');
