...
const AGENTS = [
  ...
  {
    id: 'explainability',
    dashboard: 'ExplainabilityDashboard.tsx',
    testid: 'explainability-dashboard',
  },
  {
    id: 'orchestrator',
    dashboard: 'OrchestratorDashboard.tsx',
  ...
  {
    id: 'log_analysis',
    dashboard: 'LogAnalysisDashboard.tsx',
...
for (const a of AGENTS) {
  ...
  if (!existsSync(dir)) {
    out.agents.push({
      ...
      evidence: [],
      blockers: ['service folder absent'],
      nextStep: `create src/services/${a.id}/`,
    });
    missing++;
    continue;
  }
  ...
  if (dashExists) {
    ...
    if (
      src.includes(`data-testid="${a.testid}"`) ||
      src.includes(`data-testid={'${a.testid}'}`) ||
      src.includes(`data-testid={"${a.testid}"}`)
    ) {
      testidPresent = true;
  ...
  if (signalHits === 0 && dashExists)
    blockers.push('no runtime signal source detected (invoke/subscribe/fetch)');
  ...
  if (!dashExists) {
    state = 'MISSING';
    missing++;
  } else if (signalHits > 0 && testidPresent && tests.length > 0) {
...
console.log(
  `OK agents=${AGENTS.length} proven=${proven} partial=${partial} missing=${missing} verdict=${out.verdict}`
);
