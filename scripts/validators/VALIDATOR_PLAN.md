...
| Champ              | Valeur                                                                                                                                      |
| ------------------ | ------------------------------------------------------------------------------------------------------------------------------------------- |
| **Règle**          | Persistance chat active = `chatMemoryCompactor` (`localStorage`) + `chatMemoryCompactor.flushPendingSaves()` depuis `useConversationEngine` |
| **Cause racine**   | Ancienne hypothèse validator: `memory_core_state.json` traité à tort comme source active du chat frontend                                   |
| **Statut attendu** | **PASS** (si compactor lit/écrit localStorage et si useConversationEngine flush après `saveMessage`)                                        |
