...
describe('GATE_P5: PHASE_5 Runtime Governance validations', () => {
  describe('P5.G1: Runtime isolation verification', () => {
    ...
    it('should have runtime health check scripts', () => {
      ...
      expect(existsSync(healthCheck), `Missing health check script: ${healthCheck}`).toBe(
        true
      );
    });
  });
  ...
  describe('P5.G2: Operational procedures validation', () => {
    it('should have safe-run maintenance wrapper', () => {
      ...
      expect(existsSync(safeRun), `Missing safe-run script: ${safeRun}`).toBe(true);
    });

    it('should have operational audit scripts', () => {
      const opsAudit = resolve(ROOT, 'scripts/governance/ops-audit.sh');

      expect(existsSync(opsAudit), `Missing ops audit script: ${opsAudit}`).toBe(true);
    });
  ...
  describe('P5.G3: Deployment governance checks', () => {
    it('should have evidence documentation structure', () => {
      ...
      expect(existsSync(evidenceDir), `Missing evidence directory: ${evidenceDir}`).toBe(
        true
      );

      expect(existsSync(p5Evidence), `Missing P5 evidence: ${p5Evidence}`).toBe(true);
    });

    it('should have governance audit framework', () => {
      const constitutionalAudit = resolve(
        ROOT,
        'scripts/governance/constitutional-audit.sh'
      );

      expect(
        existsSync(constitutionalAudit),
