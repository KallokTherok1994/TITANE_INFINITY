...
describe('desktop icon refresh scripts', () => {
  it('publishes multiple icon resolutions for local and system launchers', () => {
    ...
    expect(updateDesktopIconScript).toContain('copy_icon_resolution "$LOCAL_ICON_ROOT" "$resolution"');
    expect(updateDesktopIconScript).toContain('sync_system_icon_resolution "$resolution"');
  });

  it('preserves the invoking user home when the launcher is regenerated under sudo', () => {
    expect(updateDesktopIconScript).toContain('TARGET_USER_HOME="$(resolve_target_user_home)"');
    ...
    expect(updateDesktopIconScript).toContain('Exec=gnome-terminal -- tail -f $TARGET_CONFIG_DIR/logs/titane.log');
  ...
  it('refreshes the post-build system icons with gtk-update-icon-cache', () => {
    ...
    expect(postBuildDesktopIconsScript).toContain('install_system_icon_resolution "$resolution"');
    expect(postBuildDesktopIconsScript).toContain('sudo gtk-update-icon-cache -f -t /usr/share/icons/hicolor');
