...
test.describe('Chat IA - Race Conditions & Streaming', () => {
  ...
  test('RACE CONDITION: Messages multiples rapides (10 msg/2s)', async ({ page }) => {
    ...
    console.log(`✅ Test RACE CONDITION passé: ${messageCount} messages, ${count} réponses`);
  });

  test('STREAMING: Messages assistant progressifs', async ({ page }) => {
    // Envoyer un message qui génère une réponse longue
    await page.locator('textarea[placeholder*="Message"]').fill('Explique-moi la théorie de la relativité en 3 paragraphes');
    await page.keyboard.press('Enter');
  ...
  test('ERROR BOUNDARY: Message invalide ne crashe pas l\'UI', async ({ page }) => {
    ...
    await page.evaluate(() => {
      ...
      window.dispatchEvent(new CustomEvent('titane-chat-test-invalid-message', {
        detail: { message: null }
      }));
    });
    ...
    const userMessage = page.locator('.message-bubble-user', { hasText: 'Test après erreur' });
  ...
  test('VALIDATION: Messages vides sont filtrés', async ({ page }) => {
    ...
    await page.evaluate(() => {
      // @ts-ignore: Force injection pour test
      window.dispatchEvent(new CustomEvent('titane-chat-test-empty-message', {
  ...
  test('FAILSAFE: Erreur d\'envoi restaure l\'input', async ({ page }) => {
    ...
    console.log(`✅ Test FAILSAFE passé: UI stable après erreur réseau (input: "${inputValue}")`);
  });
  ...
  test('EDGE CASE: Caractères spéciaux et emojis', async ({ page }) => {
    const specialMessage = '🚀 Test <script>alert("XSS")</script> & "quotes" \'apostrophes\' \\backslash 中文 العربية 🔥';
