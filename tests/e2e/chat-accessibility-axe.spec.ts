...
test.describe('Chat IA - Accessibility WCAG 2.1 AA', () => {
  ...
  test('WCAG 2.1 AA: Bulle flottante accessible', async ({ page }) => {
    ...
    const chatBubble = page.locator('[role="button"][aria-label="Ouvrir TITANE∞ AI Companion"]');
    ...
    console.log(`✅ Bulle: ${accessibilityScanResults.passes.length} checks passés, 0 violations`);
  });
  ...
  test('WCAG 2.1 AA: Panel chat accessible', async ({ page }) => {
    ...
    const chatPanel = page.locator('[role="dialog"][aria-label="TITANE∞ AI Companion Chat Panel"]');
    ...
    console.log(`   Violations: ${accessibilityScanResults.violations.length} (mineures tolérées)`);
  });
