...
impl StabilityEngine {
    ...
    pub fn compute_convergence(&self) -> f32 {
        ...
        let mean: f32 = self.stability_history.iter().sum::<f32>() / self.stability_history.len() as f32;
        let variance: f32 = self.stability_history.iter()
            .map(|&x| (x - mean).powi(2))
            .sum::<f32>() / self.stability_history.len() as f32;
