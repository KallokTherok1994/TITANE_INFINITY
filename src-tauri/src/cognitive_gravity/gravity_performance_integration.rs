...
use crate::cognitive_gravity::{AntiAttractor, CognitiveGravityEngine, GravityField};
...
impl GravityPerformanceIntegration {
    ...
    pub async fn sync_gravity_to_performance(&self) -> TitaneResult<()> {
        ...
        if anti_attractors.overload > 0.8 {
            log::warn!(
                "🔴 High Overload detected ({:.2}) → Performance pressure",
        ...
        // 🟢 LOW OVERLOAD + HIGH STABILITY → Log optimal state
            ...
            log::info!(
                "🟢 Low Overload ({:.2}) + High Stability → Optimal performance",
                anti_attractors.overload
            );
        }
        // 🟡 MODERATE → Keep current parallelism
            ...
            log::debug!(
                "🟡 Moderate load ({:.2}) → Normal performance",
                anti_attractors.overload
            );
        }
        ...
        if field.entropy > 0.7 {
            log::warn!(
                "📊 High Entropy ({:.2}) → System complexity high",
                field.entropy
            );
        }
    ...
    pub async fn sync_performance_to_gravity(&self) -> TitaneResult<()> {
        ...
        self.gravity_engine
            .set_anti_attractor(AntiAttractor::Overload, overload_score)
            .await;
    ...
    pub async fn start_auto_sync(&self, interval_ms: u64) -> TitaneResult<()> {
        ...
        log::info!(
            "✅ Gravity-Performance auto-sync started ({}ms interval)",
            interval_ms
        );
        Ok(())
    }
