...
pub use feedback_collectors::{
    CompleteFeedback, KernelFeedback, OmegaFeedback, MemoryFeedback,
    AgentsFeedback, HarmonicFeedback, PerformanceFeedback,
};
...
impl CognitiveGravityEngine {
        ...
        *running = true;
        ...
        tokio::spawn(async move {
            ...
            while *running_clone.read().await {
                ...
                if let Err(e) = feedback_loop.feedback_cycle(
                    &mut attractors_guard,
                    &mut anti_attractors_guard,
                ).await {
                ...
                let anti_attractor_sum = anti_attractors_guard.compute_total_repulsion(&anti_attractor_weights);
    ...
    pub async fn diagnostics(&self) -> GravityDiagnostics {
        ...
        self.monitor.analyze(&*field, &*attractors, &*anti_attractors, &*density)
    }
}
...
mod tests {
    ...
    async fn test_feedback_loop_integration() {
        ...
        assert!(attractors.clarity > 0.0, "Clarity should be set from KernelFeedback");
        assert!(attractors.coherence > 0.0, "Coherence should be set from OmegaFeedback");
    ...
    async fn test_attractor_mapping() {
        ...
        assert!(attractors.clarity > 0.0, "Clarity should be set from KernelFeedback.system_health");
        assert!(attractors.coherence > 0.0, "Coherence should be set from OmegaFeedback.reflection_depth");
        assert!(attractors.alignment > 0.0, "Alignment should be set from MemoryFeedback.vector_alignment");
        assert!(attractors.focus > 0.0, "Focus should be set from AgentsFeedback.consensus_score");
        assert!(attractors.truth > 0.0, "Truth should be set from HarmonicFeedback.global_harmony");
        assert!(attractors.simplicity > 0.0, "Simplicity should be set from HarmonicFeedback.stability");
    ...
    async fn test_harmonic_gravity_feedback_integration() {
        ...
        assert!(attractors.truth > 0.0, "Truth should be influenced by HarmonicFeedback.global_harmony");
        
        // Verify Simplicity attractor is set from HarmonicFeedback
        assert!(attractors.simplicity > 0.0, "Simplicity should be influenced by HarmonicFeedback.stability");
        
        // Get field and verify coherence
        let field = engine.get_field().await;
        assert!(field.resonance >= 0.0, "Harmonic resonance should be present in field");
