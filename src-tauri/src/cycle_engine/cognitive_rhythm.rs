...
mod tests {
    ...
    fn test_cognitive_rhythm_serialization() {
        ...
        let params = CognitiveRhythmParams::from_cycle_state(&state);
        let json =
            serde_json::to_string(&params).expect("cognitive rhythm params should serialize");
        let restored: CognitiveRhythmParams =
            serde_json::from_str(&json).expect("cognitive rhythm params should deserialize");
