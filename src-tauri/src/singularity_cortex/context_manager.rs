...
impl ContextManager {
    pub async fn build_context(
        ...
        let bundle = memory.recall(query, 10).await
        ...
        let combined = Self::merge_contexts(&short_term, &mid_term, &long_term, &global, state.global_mode);
    ...
    fn format_entries(entries: &[crate::engines::unified_memory::models::MemoryEntry], label: &str) -> String {
        if entries.is_empty() {
            return String::new();
        }
        ...
        for entry in entries.iter().take(10) {
            output.push_str(&format!("- [{}] {}\n", entry.role, entry.content.chars().take(100).collect::<String>()));
        }
        output
    }
    ...
    fn format_global(context: &[String], state: &SingularityState) -> String {
        ...
        format!(
            ...
            context.iter().enumerate().map(|(i, item)| format!("{}. {}", i + 1, item.chars().take(80).collect::<String>())).collect::<Vec<_>>().join("\n")
        )
    }
    
    fn merge_contexts(stm: &str, mtm: &str, ltm: &str, global: &str, mode: CognitiveMode) -> String {
        let mut combined = String::from("## CONTEXTE UNIFIÉ TITANE∞\n\n");
        
        match mode {
            CognitiveMode::Coach => {
                if !stm.is_empty() { combined.push_str(stm); combined.push_str("\n"); }
                if !mtm.is_empty() { combined.push_str(mtm); combined.push_str("\n"); }
                if !global.is_empty() { combined.push_str(global); }
            }
            CognitiveMode::Architect => {
                if !global.is_empty() { combined.push_str(global); combined.push_str("\n"); }
                if !ltm.is_empty() { combined.push_str(ltm); combined.push_str("\n"); }
                if !mtm.is_empty() { combined.push_str(mtm); }
            }
            CognitiveMode::Analyst => {
                if !stm.is_empty() { combined.push_str(stm); combined.push_str("\n"); }
                if !mtm.is_empty() { combined.push_str(mtm); combined.push_str("\n"); }
                if !ltm.is_empty() { combined.push_str(ltm); }
            }
            CognitiveMode::Meta => {
                if !global.is_empty() { combined.push_str(global); combined.push_str("\n"); }
                if !ltm.is_empty() { combined.push_str(ltm); }
            ...
            CognitiveMode::Observer => {
                if !stm.is_empty() { combined.push_str(stm); }
            }
