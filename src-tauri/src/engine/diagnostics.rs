...
use crate::{
    types::{
        EvolutionReport, HarmoniaState, HeliosState, Issue, IssueCategory, IssueSeverity,
        NexusState, Recommendation, RepairAction, SentinelState,
    },
    ...
    pub async fn diagnose(
        ...
        if helios.cpu_usage > 90.0 {
            ...
            recommendations.push(Recommendation {
                id: Uuid::new_v4().to_string(),
                action: RepairAction::Log(
                    "CPU usage critical - consider reducing workload".to_string(),
                ),
                priority: 10,
        ...
        let failing_modules: Vec<_> = nexus
            .modules
            .values()
            .filter(|m| {
                matches!(
                    m.health,
                    crate::types::nexus::ModuleHealth::Failing
                        | crate::types::nexus::ModuleHealth::Offline
                )
            })
            .collect();
