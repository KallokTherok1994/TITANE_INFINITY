...
mod integration_tests {
    ...
    use crate::api_hub::{
        ...
        temporal_circuit_breaker::{TemporalCircuitBreaker, CircuitState},
        router::APIRouter,
        provider_registry::ProviderRegistry,
        Provider, Modality, APIRequest, RequestContent, ModelChoiceStrategy,
    };
    ...
    async fn test_complete_temporal_flow() {
        ...
        let rate_limiter = TemporalRateLimiter::new(100, 1000, 60, temporal_adapter.clone());
        ...
        cache.set(key.to_string(), "cached_response".to_string(), "/test".to_string()).await;

        let cached_value = cache.get(key).await;
        assert_eq!(cached_value, Some("cached_response".to_string()), "Should retrieve cached value");
    ...
    async fn test_adaptive_rate_limiting() {
        let temporal_adapter = Arc::new(TemporalApiAdapter::new());
        let rate_limiter = TemporalRateLimiter::new(10, 100, 60, temporal_adapter.clone());
    ...
    async fn test_adaptive_cache_ttl() {
        ...
        cache.set("chat_key".to_string(), "chat_response".to_string(), "/chat".to_string()).await;

        // Stocker valeur pour endpoint embedding (TTL long)
        cache.set("embed_key".to_string(), "embed_response".to_string(), "/embedding".to_string()).await;
    ...
    async fn test_temporal_provider_suggestion() {
        ...
        assert!(!suggestion.reason.is_empty(), "Should have suggestion reason");
    ...
    async fn test_full_api_request_pipeline() {
        ...
        let rate_limiter = TemporalRateLimiter::new(100, 1000, 60, temporal_adapter.clone());
        let cache: TemporalCache<String> = TemporalCache::new(100, temporal_adapter.clone());
        let breaker = TemporalCircuitBreaker::new("/api/chat".to_string(), temporal_adapter.clone());
        let router = APIRouter::new().with_temporal_adapter(temporal_adapter.clone());

        // 1. Vérifier rate limit
        assert!(rate_limiter.acquire_permit().await.is_ok(), "Rate limit should allow");

        // 2. Vérifier circuit breaker
        assert!(breaker.allow_request().await.is_ok(), "Circuit should be closed");
        ...
        cache.set("request_123".to_string(), "response".to_string(), "/chat".to_string()).await;
