...
pub async fn persistent_memory_promote_entry(entry_id: String) -> Result<(), TitaneError> {
    ...
    let mut entries = PERSISTENT_ENTRIES.lock().map_err(|e| {
        TitaneError::InternalError(format!("Failed to lock PERSISTENT_ENTRIES: {}", e))
    })?;

    if let Some(entry) = entries.iter_mut().find(|e| e.id == entry_id) {
        entry
            .metadata
            .insert("promoted".to_string(), "true".to_string());
        log::info!("[PERSISTENT_MEMORY] ✅ Promoted entry '{}'", entry_id);
        Ok(())
    } else {
        Err(TitaneError::InternalError(format!(
            "Entry not found: {}",
            entry_id
        )))
    }
...
pub async fn persistent_memory_add_to_bundle(
    ...
    log::info!(
        "[PERSISTENT_MEMORY] add_to_bundle: {} entries → bundle {}",
        entry_ids.len(),
        bundle_id
    );

    let mut bundles = MEMORY_BUNDLES
        .lock()
    ...
    if let Some(bundle) = bundles.iter_mut().find(|b| b.id == bundle_id) {
        ...
        log::info!(
            "[PERSISTENT_MEMORY] ✅ Added entries to bundle '{}'",
            bundle_id
        );
        Ok(())
    } else {
