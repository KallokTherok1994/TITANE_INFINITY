...
/// Ping Gemini provider (via backend) - v∞ with SecureSecretsEngine
async fn ping_gemini_internal(secrets: Option<&SecureSecretsEngine>) -> ProviderStatus {
    let start = std::time::Instant::now();
    let now = now_ms();

    // Check if GEMINI_API_KEY is set (either from secrets engine or env var)
...
pub async fn orchestration_ping_providers() -> CommandResult<MultiAIState> {
    // Ping all providers (Note: no SecureSecretsEngine available here, fallback to env var)
    let gemini = ping_gemini_internal(None).await;
...
// ═══════════════════════════════════════════════════════════════════════════
// ADDITIONAL HELPER COMMANDS FOR ADMIN ENGINE
// ═══════════════════════════════════════════════════════════════════════════

/// Simple ping command for Gemini (for admin engine diagnostics) - v∞ SecureSecretsEngine
#[tauri::command]
...
/// Simple ping command for Ollama (for admin engine diagnostics)
#[tauri::command]
...
// ═══════════════════════════════════════════════════════════════════════════
// GOOGLE CLOUD SERVICES - GEMINI API v∞ (22 services activés)
// ═══════════════════════════════════════════════════════════════════════════
...
pub async fn test_gemini_services(
    ...
    let api_key = match secrets.get_secret("gemini_api_key") {
        Ok(Some(key)) => key,
        _ => {
            return Ok(GeminiFullStatus {
    ...
    let mut services = vec![
        GeminiServiceStatus {
            name: "Generative Language API (Core)".into(),
            available: false,
            endpoint: "generativelanguage.googleapis.com".into(),
            error: None,
        },
        GeminiServiceStatus {
            name: "Vertex AI API".into(),
            available: false,
            endpoint: "aiplatform.googleapis.com".into(),
            error: Some("Requires OAuth2 - Not tested".into()),
        },
        GeminiServiceStatus {
            name: "Gemini for Google Cloud API".into(),
            available: false,
            endpoint: "gemini.googleapis.com".into(),
            error: Some("Requires OAuth2 - Not tested".into()),
        },
        GeminiServiceStatus {
            name: "Data Analytics API with Gemini".into(),
            available: false,
            endpoint: "dataanalytics.googleapis.com".into(),
            error: Some("Requires OAuth2 - Not tested".into()),
        },
        GeminiServiceStatus {
            name: "Gemini Cloud Assist API".into(),
            available: false,
            endpoint: "cloudassist.googleapis.com".into(),
            error: Some("Requires OAuth2 - Not tested".into()),
        },
        GeminiServiceStatus {
            name: "Gemini Code Assist Management API".into(),
            available: false,
            endpoint: "codeassist.googleapis.com".into(),
            error: Some("Requires OAuth2 - Not tested".into()),
        },
    ];
    ...
    Ok(GeminiFullStatus {
        core_api_available: services[0].available,
        api_key_configured: true,
        total_services: 22,
        services_tested: services,
        global_latency_ms: latency,
    })
}
