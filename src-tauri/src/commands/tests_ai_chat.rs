...
mod tests {
    ...
    fn test_ai_chat_state_flags() -> Result<(), Box<dyn Error>> {
        ...
        let is_speaking = state.state_flags.get("is_speaking")
    ...
    fn test_conversations_insert_and_retrieve() -> Result<(), Box<dyn Error>> {
        ...
        state.conversations.insert("conv1".to_string(), conv.clone());
        
        let retrieved = state.conversations.get("conv1")
    ...
    async fn test_speak_too_long_text() -> Result<(), Box<dyn Error>> {
        ...
        assert!(result.is_err());
        if let Err(e) = result {
            assert!(e.contains("too long") || e.contains("max"), 
