...
impl EmotionDetector {
    ...
    fn calculate_intensity(features: &AudioFeatures) -> f32 {
        ...
        (variance_factor * 0.3 + energy_factor * 0.5 + rate_factor * 0.2).clamp(0.0, 1.0)
    }

    /// Détecte l'émotion primaire
    fn detect_primary_emotion(valence: f32, intensity: f32, features: &AudioFeatures) -> Emotion {
    ...
    fn calculate_confidence(features: &AudioFeatures) -> f32 {
        ...
        let variance_confidence = if features.pitch_variance > 10.0 {
            0.3
        } else {
            0.1
        };
    ...
    pub fn detect_from_text(text: &str) -> EmotionalState {
        ...
        let negative_count = negative_words
            ...
            .filter(|w| text_lower.contains(*w))
            .count();
        let excited_count = excited_words
            .iter()
            .filter(|w| text_lower.contains(*w))
            .count();
        let anxious_count = anxious_words
            .iter()
            .filter(|w| text_lower.contains(*w))
            .count();

        let valence = (positive_count as f32 - negative_count as f32) / 5.0;
        let intensity = ((positive_count + negative_count + excited_count + anxious_count) as f32
            / 5.0)
            .min(1.0);
