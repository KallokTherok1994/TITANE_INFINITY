...
mod tests {
    ...
    fn create_test_context(hour: u8, day_of_week: u8, season: Season) -> TemporalContext {
        TemporalContext {
            ...
            planned_tasks: vec![],
            predictions: vec![],
            alignment_status: crate::temporal_engine::long_term_alignment::AlignmentStatus {
                overall_score: 0.9,
                misalignments: vec![],
                recommendations: vec![],
            },
    ...
    fn test_kernel_peak_hours_adjustments() {
        ...
        assert!(adjustments.priority_multiplier > 1.0, "Peak hours should boost priority");
        assert!(adjustments.max_concurrent_jobs >= 7, "Peak hours should allow more jobs");
    }
    ...
    fn test_kernel_night_adjustments() {
        ...
        assert!(adjustments.priority_multiplier < 1.0, "Night should reduce priority");
        assert!(adjustments.max_concurrent_jobs <= 4, "Night should limit jobs");
    }
    ...
    fn test_kernel_resource_limits_peak() {
        ...
        assert!(limits.max_cpu_percent > 70, "Peak hours should allow high CPU");
        assert!(limits.max_memory_mb > 5000, "Peak hours should allow high memory");
    }
    ...
    fn test_kernel_maintenance_window() {
        ...
        assert!(maintenance.should_perform, "Night should recommend maintenance");
        assert_eq!(maintenance.urgency, crate::temporal_engine::integrations::kernel_integration::MaintenanceUrgency::High);
        ...
        assert!(!maintenance_day.should_perform, "Day should not recommend maintenance");
    }
    ...
    fn test_omega_depth_multiplier_peak() {
        ...
        assert_eq!(adjustments.depth_multiplier, 1.0, "Peak hours should have max depth");
        assert!(adjustments.reflection_intensity > 0.7, "Peak should enable reflection");
    }
    ...
    fn test_omega_depth_multiplier_night() {
        ...
        assert!(adjustments.depth_multiplier < 0.5, "Night should reduce depth");
    }
    ...
    fn test_omega_routing_strategy() {
        ...
        assert_eq!(strategy, RoutingStrategy::DeepAnalysis, "Peak + complex = deep analysis");
        ...
        assert_eq!(strategy_fast, RoutingStrategy::FastTrack, "Midday + simple = fast track");
    }
    ...
    fn test_omega_engine_weights() {
        ...
        assert_eq!(adjustments.engine_weights.len(), 10, "Should have 10 engine weights");
        assert!(adjustments.engine_weights[0] > 0.0, "All weights should be positive");
    }
    ...
    fn test_memory_consolidation_intensity() {
        ...
        assert!(adjustments.consolidation_intensity > 0.8, "Night should have high consolidation");

        let context_day = create_test_context(14, 3, Season::Summer);
        let adjustments_day = TemporalMemoryBridge::get_memory_adjustments(&context_day);

        assert!(adjustments_day.consolidation_intensity < 0.6, "Day should have lower consolidation");
    }
    ...
    fn test_memory_stm_ltm_threshold() {
        ...
        assert!(adjustments.stm_to_ltm_threshold < 0.5, "Night should promote easily");
    }
    ...
    fn test_memory_consolidation_recommendation() {
        ...
        assert!(!rec_day.should_run, "Day should not recommend consolidation");
    }
    ...
    fn test_memory_preload_patterns() {
        ...
        assert!(!patterns.is_empty(), "Workday morning should suggest preloading");
    }
    ...
    fn test_agi_meta_learning_intensity() {
        ...
        assert_eq!(adjustments.meta_learning_intensity, 1.0, "Night should max meta-learning");

        let context_midday = create_test_context(12, 3, Season::Summer);
        let adjustments_day = TemporalAgiBridge::get_agi_adjustments(&context_midday);

        assert!(adjustments_day.meta_learning_intensity < 0.5, "Midday should reduce meta-learning");
    }
    ...
    fn test_agi_exploration_ratio() {
        ...
        assert_eq!(adjustments.exploration_vs_exploitation, 0.3, "Peak should exploit");

        let context_night = create_test_context(3, 3, Season::Summer);
        let adjustments_night = TemporalAgiBridge::get_agi_adjustments(&context_night);

        assert_eq!(adjustments_night.exploration_vs_exploitation, 0.7, "Night should explore");
    }
    ...
    fn test_agi_seasonal_adaptation() {
        ...
        assert_eq!(adjustments.heuristic_adaptation_rate, 0.8, "Spring should adapt fast");
        ...
        assert_eq!(adjustments_winter.heuristic_adaptation_rate, 0.3, "Winter should be conservative");
    }
    ...
    fn test_conversation_verbosity() {
        ...
        assert!(adj_midday.verbosity < adj_peak.verbosity, "Midday should be more concise");
    }

    #[test]
    fn test_conversation_formality_weekend() {
        let context_weekday = create_test_context(14, 3, Season::Summer);
        let adj_weekday = TemporalConversationBridge::get_conversation_adjustments(&context_weekday);

        let context_weekend = create_test_context(14, 6, Season::Summer);
        let adj_weekend = TemporalConversationBridge::get_conversation_adjustments(&context_weekend);

        assert!(adj_weekend.formality < adj_weekday.formality, "Weekend should be less formal");
    }

    #[test]
    fn test_conversation_strategies() {
        let context_morning = create_test_context(8, 2, Season::Summer);
        let strategies = TemporalConversationBridge::suggest_conversation_strategies(&context_morning);
    ...
    fn test_conversation_temporal_narrative() {
        ...
        assert!(!narrative.current_thread.is_empty(), "Should have narrative thread");
        assert!(!narrative.temporal_markers.is_empty(), "Should have temporal markers");
