...
impl DocumentValidator {
    ...
    pub fn validate(
        ...
        match config.doc_type {
            ...
            DocumentType::BookChapter | DocumentType::Article => {
                self.validate_editorial_document(
                    content,
                    &mut errors,
                    &mut warnings,
                    &mut suggestions,
            ...
            DocumentType::Architecture | DocumentType::APIDoc => {
                self.validate_technical_document(
                    content,
                    &mut errors,
                    &mut warnings,
                    &mut suggestions,
    ...
    fn validate_legal_document(
        ...
        if let Some(clauses) = &content.mandatory_clauses {
            if clauses.is_empty() {
                warnings.push(ValidationWarning {
                    ...
                    suggestion: Some(
                        "Ajouter les clauses essentielles (confidentialité, responsabilité, etc.)"
                            .to_string(),
                    ),
                });
            }
            ...
            let has_confidentiality = clauses
                .iter()
                .any(|c| matches!(c.category, ClauseCategory::Confidentiality));
            let has_liability = clauses
                .iter()
                .any(|c| matches!(c.category, ClauseCategory::Liability));
        ...
        let section_titles: Vec<String> = content
            .sections
            .iter()
        ...
        if !section_titles
            .iter()
            .any(|t| t.contains("responsabilit") || t.contains("liability"))
        {
            suggestions.push(Suggestion {
                category: "legal".to_string(),
        ...
        if !section_titles
            .iter()
            .any(|t| t.contains("résiliation") || t.contains("termination") || t.contains("durée"))
        {
            suggestions.push(Suggestion {
                category: "legal".to_string(),
    ...
    fn validate_editorial_document(
        ...
        if content.sections.len() < 3 {
            suggestions.push(Suggestion {
                ...
                message: "Développer davantage le contenu avec des sections supplémentaires"
                    .to_string(),
                priority: 6,
            });
        }
        ...
        if !has_examples {
            suggestions.push(Suggestion {
                category: "editorial".to_string(),
                message: "Ajouter des exemples ou cas pratiques pour illustrer les concepts"
                    .to_string(),
                priority: 7,
            });
        }
    ...
    fn validate_technical_document(
        ...
        if !has_diagrams {
            suggestions.push(Suggestion {
                ...
                message: "Ajouter des diagrammes ou schémas pour illustrer l'architecture"
                    .to_string(),
                priority: 8,
            });
        }
        ...
        if !has_code {
            warnings.push(ValidationWarning {
                ...
                suggestion: Some(
                    "Ajouter des exemples de code pour faciliter la compréhension".to_string(),
                ),
            });
        }
