...
impl StorageEngine {
    ...
    pub async fn save(&self, document: &Document) -> Result<String> {
        ...
        fs::create_dir_all(&self.storage_path).map_err(|e| {
            DocEngineError::StorageError(format!("Impossible de créer le répertoire: {}", e))
        })?;
    ...
    pub async fn load(&self, document_id: &str) -> Result<Document> {
        ...
        if !file_path.exists() {
            return Err(DocEngineError::StorageError(format!(
        ...
        // Désérialisation
        let document: Document = serde_json::from_str(&json_data).map_err(|e| {
            DocEngineError::StorageError(format!("Erreur de désérialisation: {}", e))
        })?;
    ...
    pub async fn list_documents(&self) -> Result<Vec<DocumentMetadata>> {
        ...
        let data = fs::read_to_string(&metadata_path).map_err(|e| {
            DocEngineError::StorageError(format!("Erreur de lecture métadonnées: {}", e))
        })?;

        let metadata_list: Vec<DocumentMetadata> = serde_json::from_str(&data).map_err(|e| {
            DocEngineError::StorageError(format!("Erreur de désérialisation métadonnées: {}", e))
        })?;
    ...
    pub async fn delete(&self, document_id: &str) -> Result<()> {
        ...
        if file_path.exists() {
            fs::remove_file(&file_path).map_err(|e| {
                DocEngineError::StorageError(format!("Erreur de suppression: {}", e))
            })?;
        }
    ...
    fn encrypt_data(&self, data: &[u8]) -> Result<Vec<u8>> {
        ...
        let password_hash = argon2.hash_password(password, &salt).map_err(|e| {
            DocEngineError::StorageError(format!("Erreur de dérivation de clé: {}", e))
        })?;
        ...
        let cipher = Aes256Gcm::new_from_slice(key).map_err(|e| {
            DocEngineError::StorageError(format!("Erreur de création du cipher: {}", e))
        })?;
    ...
    fn save_metadata(&self, document: &Document) -> Result<()> {
        ...
        if let Some(pos) = metadata_list
            .iter()
            .position(|m| m.id == document.metadata.id)
        {
        ...
        let json = serde_json::to_string_pretty(&metadata_list).map_err(|e| {
            DocEngineError::StorageError(format!("Erreur de sérialisation métadonnées: {}", e))
        })?;

        fs::write(&metadata_path, json).map_err(|e| {
            DocEngineError::StorageError(format!("Erreur d'écriture métadonnées: {}", e))
        })?;
    ...
    fn remove_from_metadata(&self, document_id: &str) -> Result<()> {
        ...
        let mut metadata_list: Vec<DocumentMetadata> =
