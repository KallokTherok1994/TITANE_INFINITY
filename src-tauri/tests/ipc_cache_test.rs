...
async fn test_cache_async_operations() {
    ...
    let value1 = cache.get_or_compute_async("async_key", || async {
    ...
    let value2 = cache.get_or_compute_async("async_key", || async {
...
async fn test_cache_concurrent_access() {
    ...
    for i in 0..10 {
        ...
        let handle = tokio::spawn(async move {
            cache_clone.get_or_compute_async(&format!("key{}", i % 3), || async {
                sleep(Duration::from_millis(10)).await;
                i * 10
            }).await
        });
...
async fn test_cache_concurrent_same_key() {
    ...
    for _ in 0..10 {
        ...
        let handle = tokio::spawn(async move {
            cache_clone.get_or_compute_async("shared_key", || async {
