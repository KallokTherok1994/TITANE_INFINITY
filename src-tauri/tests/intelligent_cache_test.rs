...
async fn test_cache_expiration() {
    ...
    assert!(cache.get::<serde_json::Value>(&key).is_some(), "Should hit immediately");
    
    // Wait for TTL to expire
    tokio::time::sleep(Duration::from_millis(150)).await;
    
    // After TTL: should miss
    assert!(cache.get::<serde_json::Value>(&key).is_none(), "Should miss after TTL");
...
async fn test_lru_eviction() {
    ...
    assert!(cache.get::<serde_json::Value>(&key1).is_none(), "Entry 1 should be evicted (LRU)");
    assert!(cache.get::<serde_json::Value>(&key0).is_some(), "Entry 0 should be preserved (recently used)");
    assert!(cache.get::<serde_json::Value>(&key3).is_some(), "Entry 3 should exist (just added)");
...
async fn test_concurrent_cache_access() {
    ...
    assert!(metrics.hits() + metrics.misses() == metrics.total_queries(), "Hits + misses should equal total");
}
...
async fn test_cache_hit_rate() {
    ...
    // First query: miss
    assert!(cache.get::<serde_json::Value>(&key).is_none(), "First query should miss");
    ...
    for _ in 0..10 {
        assert!(cache.get::<serde_json::Value>(&key).is_some(), "Subsequent queries should hit");
    }
    ...
    assert!((hit_rate - 0.909).abs() < 0.01, "Hit rate should be ~90.9% (10/11)");
}
...
async fn test_invalidate_pattern() {
    ...
    for i in 0..5 {
        let key = CacheKey::new(&format!("get_user_{}", i), json!({}));
        assert!(cache.get::<serde_json::Value>(&key).is_none(), "User entries should be invalidated");
    }
    
    // Config entries should still exist
    for i in 0..3 {
        let key = CacheKey::new(&format!("get_config_{}", i), json!({}));
        assert!(cache.get::<serde_json::Value>(&key).is_some(), "Config entries should remain");
    }
}
...
async fn test_cache_persistence() {
    ...
    // Create cache, add data, persist
    {
        ...
        let key = CacheKey::new("cmd", json!({"id": 1}));
        cache.set(key, json!({"data": "test_value"}), Duration::from_secs(3600));
    ...
    // Create new cache instance, restore from disk
    {
        ...
        let key = CacheKey::new("cmd", json!({"id": 1}));
        let value: serde_json::Value = cache.get(&key).expect("Restored cache should contain value");
...
async fn test_cache_clear() {
    ...
    for i in 0..10 {
        let key = CacheKey::new("cmd", json!({"id": i}));
        assert!(cache.get::<serde_json::Value>(&key).is_none(), "Cache should be empty after clear");
    }
}
