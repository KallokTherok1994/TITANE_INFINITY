...
mod unified_memory_tests {
    ...
    async fn test_semantic_search() {
        ...
        let has_ai_content = bundle.stm.iter().any(|e| 
            e.content.contains("intelligence") || 
            e.content.contains("learning")
        );
    ...
    async fn test_embedding_generation() {
        ...
        assert!((norm - 1.0).abs() < 0.1, "Should be approximately normalized");
    }
    ...
    async fn test_summarization() {
        ...
        assert!(!summary.is_empty() || engine.stats().mtm_count > 0, 
